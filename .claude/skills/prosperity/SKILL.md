...
- **Layer 1 — Joint state generator** (`rust_simulator/src/scenario.rs` trait, `rust_simulator/src/scenarios/r5.rs` impl). Produces FV paths for all 50 assets *jointly* (Pebble basket sum = 50,000 exactly per tick; SNACKPACK_CHOC + SNACKPACK_VANILLA ≈ K_day where K_day is its own slow OU process; SNACKPACK_PISTACHIO/STRAWBERRY/RASPBERRY share a 1-factor model `F_i = OU_i + ℓ_i · K_triplet` with loadings ~[−0.40, −0.66, +0.64] capturing the historical pairwise corr +0.91 / −0.92 / −0.83; 44 free OU walks elsewhere) plus a list of **shared Poisson pulses** for 3 groups (V = 40 vanilla products, P = 5 pebbles, M = 5 microchips). A single pulse fires its whole group on the same side at the same quantity in the same tick.
...
**Key files for R5:**
- `analysis/round5/rigorous_calibration.py` — produces `analysis/round5/calibration_r5.json` from historical CSVs **and** auto-invokes `snackpack_triplet_factor.py` at the end to refresh the triplet block in `scenario_params.json`. Run this single script to regenerate everything.
- `analysis/round5/snackpack_triplet_factor.py` — fits the 1-factor model for {PIS, STRAW, RASP} and patches `calibration/r5/scenario_params.json` (adds top-level `snackpack_triplet` block + replaces `sigma` with `sigma_idio` for the 3 members; original sigma preserved as `sigma_total_pre_factor`). Idempotent. The patched file backs up the previous version to `scenario_params.json.bak` on first run only.
