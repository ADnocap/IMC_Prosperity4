"""Madden — Steve Madden, the guy who size matters more than finesse.
Hypothesis test on top of harry_potter_v4 (portal sub 370288, +17,449):
push every aggression knob harder while keeping the EMA core unchanged.

If harry_v4's MR signal is real on portal (it is — +17,449 proves it),
then sizing it up should produce proportionally more PnL. The only risk
is over-trading at thin signals; the higher MR_TAKE_Z guards against
that for cross-spread takes.

Changes vs harry_v4:
  - MR_K            0.55 -> 0.75   (target position scales harder with z)
  - MR_MAX_FRAC     0.85 -> 0.95   (allow nearly-full position at strong z)
  - MR_TAKE_Z       1.2  -> 1.0    (cross-spread take fires earlier)
  - MR_TAKE_MAX     40   -> 70     (bigger single-tick taking)
  - MR_MM_LEVEL_SIZE 30  -> 45     (bigger passive quotes per level)
...
class Trader:
    ...
    VAR_ALPHA = 0.01
    MR_K = 0.75              # was 0.55
    MR_MAX_FRAC = 0.95       # was 0.85
    MR_MIN_N = 50
    MR_MM_LEVEL_SIZE = 45    # was 30
    MR_MM_LEVELS = 2
    MR_TAKE_Z = 1.0          # was 1.2
    MR_TAKE_MAX = 70         # was 40
