"""Wigwam — Robbie Feinberg, the wild Brooklyn guy who jumps on
opportunities others miss. Hypothesis test on top of harry_potter_v4
(portal sub 370288, +17,449): add EMA mean-reversion on HYDROGEL_PACK.

Why: harry_v4 does OBI-tilted MM on HYDROGEL, but
analysis/round3/multi_tick_signals.md found HYDROGEL z>1 reversion at
H=200 prints +3.68 ticks/sig (Sharpe +11.4, ~5400 sigs/day) — the
single strongest non-OBI signal in the dataset. MR captures it
directly; OBI MM doesn't.

HYDROGEL has a wider bid-ask spread (~15-19 ticks vs ~3 on VELVET) so:
  - Each MR fill is much more profitable per share (wider spread = bigger edge)
  - But cross-spread takes are expensive too (~7-8 tick half-spread cost)
  - Use a higher z threshold for taking (MR_TAKE_Z = 1.5 vs 1.2)
  - Slightly slower EMA than VELVET (alpha 0.0003 vs 0.0005) — HYDROGEL has
    higher per-tick sigma (~1.92 vs 0.96), so more averaging needed

Everything else identical to harry_v4. VELVET + VEV_5000-5500 still on
EMA MR. VEV_4000/4500 still on OBI MM. Dead VEV_6000/6500 still skipped.
"""
...
FALLBACK_FV: Dict[str, float] = {
    HYDROGEL: 10010.0, VELVETFRUIT: 5250.0,
...
# Per-asset MR overrides (HYDROGEL needs slower EMA + higher z threshold).
HYDROGEL_EMA_ALPHA = 0.0003
HYDROGEL_MR_TAKE_Z = 1.5
HYDROGEL_MM_LEVEL_SIZE = 25  # slightly smaller per-level to allow more layers
HYDROGEL_MM_LEVELS = 3

MR_ASSETS_DEFAULT = (VELVETFRUIT, VEV_5000, VEV_5100, VEV_5200,
                     VEV_5300, VEV_5400, VEV_5500)
MR_ASSETS_HYDROGEL = (HYDROGEL,)
...
class Trader:
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product in MR_ASSETS_HYDROGEL:
                result[product] = self._trade_mr(
                    product, od, pos, td,
                    ema_alpha=HYDROGEL_EMA_ALPHA,
                    take_z=HYDROGEL_MR_TAKE_Z,
                    level_size=HYDROGEL_MM_LEVEL_SIZE,
                    mm_levels=HYDROGEL_MM_LEVELS,
                )
            elif product in MR_ASSETS_DEFAULT:
                result[product] = self._trade_mr(
                    product, od, pos, td,
                    ema_alpha=self.EMA_ALPHA,
                    take_z=self.MR_TAKE_Z,
                    level_size=self.MR_MM_LEVEL_SIZE,
                    mm_levels=self.MR_MM_LEVELS,
                )
    ...
    def _trade_mr(self, product: str, od: OrderDepth, pos: int, td: dict,
                  *, ema_alpha: float, take_z: float, level_size: int,
                  mm_levels: int) -> List[Order]:
        ...
        fv, sigma, n = self._update_ema(td, product, mid, ema_alpha)
        ...
        big_side_size = level_size
        small_side_size = max(5, level_size // 3)
    ...
    def _update_ema(self, td: dict, product: str, mid: float,
        ...
        new_ema = prev_ema + alpha * residual
