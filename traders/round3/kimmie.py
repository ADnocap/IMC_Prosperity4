"""Kimmie — Kimmie Belzer, the secretary who put away every spare slip
of paper. Hypothesis test on top of harry_potter_v4 (portal sub 370288,
+17,449): keep harry's signal logic intact, but stack passive quotes
DEEPER (more levels, smaller per-level) so we capture more of the bot
flow that walks past the inside.

Why: harry_v4 posts at 2 levels per side (best_bid+1 / best_bid+2 etc.)
with 30 lots each. Bots walk through the spread occasionally; deeper
ladders queue more quote-priority and catch more fills.

Changes vs harry_v4:
  - MR_MM_LEVELS     2  -> 4    (twice the queue depth)
  - MR_MM_LEVEL_SIZE 30 -> 18   (smaller per level so total exposure stays
                                  bounded — 4 levels x 18 = 72 vs old 2 x 30 = 60)

Position-limit math: with 4 levels at size 18, worst-case queued buy is
72. With pos=200 (limit), buy_room=0, so all queued buys are dropped to 0
by the existing `min(level_size, remaining_room)` logic — no breach.

Everything else (MR signal, OBI MM, dead-options skip) identical.
"""
...
class Trader:
    ...
    MR_MAX_FRAC = 0.85
    MR_MIN_N = 50
    MR_MM_LEVEL_SIZE = 18    # was 30
    MR_MM_LEVELS = 4         # was 2
    MR_TAKE_Z = 1.2
    MR_TAKE_MAX = 40
