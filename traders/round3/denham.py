"""Denham — Patrick Denham, the FBI agent who follows the truth wherever
it leads. Hypothesis test on top of harry_potter_v4 (portal sub 370288,
+17,449): replace the slow-EMA voucher fair value with Black-Scholes
priced from the underlying VELVETFRUIT_EXTRACT mid.

Why: harry_v4's MR uses EMA-of-mid (half-life ~1,386 ticks) as the FV.
When VELVET moves, the BS-implied voucher fair value should jump
immediately by ~delta * deltaS, but the EMA lags and MR fades the
legitimate move. BS-from-underlying gives the true cross-asset FV.

VELVET itself stays on EMA MR (it's the underlying — no BS to apply).
HYDROGEL + VEV_4000/4500 stay on OBI-tilted MM (their FV processes are
not coupled to VELVET in any simple way).

Smile coefficients fitted on Day 2 historical data
(analysis/round3/smile_coefs_day2.json).

Caveat: MC backtester decouples vouchers from VELVET, so this trader will
look BAD in MC but should look BETTER on portal. Validate via portal sub
only.
"""

try:
    from datamodel import Order, OrderDepth, TradingState
except ImportError:
    from backtester.datamodel import Order, OrderDepth, TradingState

from typing import Dict, List, Optional, Tuple
from statistics import NormalDist
import json
import math


HYDROGEL = "HYDROGEL_PACK"
VELVETFRUIT = "VELVETFRUIT_EXTRACT"
VEV_4000 = "VEV_4000"
VEV_4500 = "VEV_4500"
VEV_5000 = "VEV_5000"
VEV_5100 = "VEV_5100"
VEV_5200 = "VEV_5200"
VEV_5300 = "VEV_5300"
VEV_5400 = "VEV_5400"
VEV_5500 = "VEV_5500"

LIMITS: Dict[str, int] = {
    HYDROGEL: 200, VELVETFRUIT: 200,
    VEV_4000: 300, VEV_4500: 300, VEV_5000: 300, VEV_5100: 300,
    VEV_5200: 300, VEV_5300: 300, VEV_5400: 300, VEV_5500: 300,
}

VEV_STRIKES: Dict[str, int] = {
    VEV_5000: 5000, VEV_5100: 5100, VEV_5200: 5200,
    VEV_5300: 5300, VEV_5400: 5400, VEV_5500: 5500,
}

# Hard-coded smile (from analysis/round3/smile_coefs_day2.json).
# iv(m) = SMILE_A + SMILE_B * m + SMILE_C * m**2,  m = log(K/S)
SMILE_A = 0.23191586677818826
SMILE_B = 0.04357235171166418
SMILE_C = 1.9376638984839945
TTE_YEARS = 6.0 / 365.0
IV_FLOOR = 0.05

FALLBACK_VELVET_FV = 5250.0
FALLBACK_VELVET_SIGMA = 15.0

# Per-strike fallback sigma for the variance-EMA seed (from calibration md).
FALLBACK_VOUCHER_SIGMA: Dict[str, float] = {
    VEV_5000: 14.0, VEV_5100: 13.0, VEV_5200: 10.0,
    VEV_5300: 6.0, VEV_5400: 3.4, VEV_5500: 1.7,
}

# MR routing: VELVET uses EMA-mid (own random walk), vouchers use BS-from-S.
MR_VELVET_ASSETS = (VELVETFRUIT,)
MR_VOUCHER_ASSETS = tuple(VEV_STRIKES.keys())
VEV_MM_ASSETS = (VEV_4000, VEV_4500)
OBI_MM_ASSETS = (HYDROGEL,)


_NORM = NormalDist()


def bs_call(S: float, K: float, T: float, sigma: float) -> float:
    if T <= 0 or sigma <= 0:
        return max(S - K, 0.0)
    sqrtT = math.sqrt(T)
    d1 = (math.log(S / K) + 0.5 * sigma * sigma * T) / (sigma * sqrtT)
    d2 = d1 - sigma * sqrtT
    return S * _NORM.cdf(d1) - K * _NORM.cdf(d2)


def smile_iv(K: float, S: float) -> float:
    m = math.log(K / S)
    iv = SMILE_A + SMILE_B * m + SMILE_C * m * m
    return max(iv, IV_FLOOR)


class Trader:
    EMA_ALPHA = 0.0005
    VAR_ALPHA = 0.01
    MR_K = 0.55
    MR_MAX_FRAC = 0.85
    MR_MIN_N = 50
    MR_MM_LEVEL_SIZE = 30
    MR_MM_LEVELS = 2
    MR_TAKE_Z = 1.2
    MR_TAKE_MAX = 40
    MR_TIGHT_SPREAD_MIN = 2

    VEV_BASE_SIZE = 15
    VEV_TIGHT_SPREAD_MIN = 2
    VEV_SOFT_POS_FRAC = 0.6
    OBI_SKEW_1 = 0.1
    OBI_SKEW_2 = 0.4
    OBI_SKEW_3 = 0.7
    VEV_SIZES_MILD = (22, 8)
    VEV_SIZES_STRONG = (30, 2)
    VEV_SIZES_EXTREME = (40, 3)

    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        td: dict = self._parse_td(state.traderData)
        result: Dict[str, List[Order]] = {}

        # Get the underlying mid first — needed by every BS-priced voucher.
        S = self._underlying_mid(state.order_depths.get(VELVETFRUIT))

        for product in state.order_depths:
            od: OrderDepth = state.order_depths[product]
            pos = state.position.get(product, 0)
            if product in MR_VOUCHER_ASSETS and S is not None:
                result[product] = self._trade_voucher_bs(product, od, pos, td, S)
            elif product in MR_VELVET_ASSETS:
                result[product] = self._trade_mr_velvet(product, od, pos, td)
            elif product in VEV_MM_ASSETS or product in OBI_MM_ASSETS:
                result[product] = self._trade_vev_mm(product, od, pos)
            else:
                result[product] = []
        return result, 0, json.dumps(td)

    @staticmethod
    def _underlying_mid(od: Optional[OrderDepth]) -> Optional[float]:
        if od is None:
            return None
        if not od.buy_orders or not od.sell_orders:
            return None
        bb = max(od.buy_orders.keys())
        ba = min(od.sell_orders.keys())
        return (bb + ba) / 2.0

    # ---- Voucher MR using BS-from-underlying as FV ----------------------
    def _trade_voucher_bs(self, product: str, od: OrderDepth, pos: int,
                          td: dict, S: float) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        best_bid = bids[0]
        best_ask = asks[0]
        if best_ask - best_bid < self.MR_TIGHT_SPREAD_MIN:
            return []
        mid = (best_bid + best_ask) / 2.0

        K = VEV_STRIKES[product]
        iv = smile_iv(K, S)
        fv = bs_call(S, K, TTE_YEARS, iv)

        # Variance EMA still tracked observationally (for sigma).
        var_key = f"var_{product}"
        n_key = f"n_{product}"
        ema_key = f"ema_{product}"  # tracks (mid - fv) residual scale
        prev_var = td.get(var_key, FALLBACK_VOUCHER_SIGMA.get(product, 8.0) ** 2)
        prev_resid_ema = td.get(ema_key, 0.0)
        n = td.get(n_key, 0)
        residual = mid - fv
        new_resid_ema = prev_resid_ema + self.VAR_ALPHA * (residual - prev_resid_ema)
        new_var = prev_var + self.VAR_ALPHA * (residual * residual - prev_var)
        td[var_key] = max(1.0, new_var)
        td[ema_key] = new_resid_ema
        td[n_key] = n + 1
        sigma = max(1.0, new_var) ** 0.5

        limit = LIMITS[product]
        if n + 1 < self.MR_MIN_N or sigma <= 0:
            target = 0
            z = 0.0
        else:
            z = (mid - fv) / sigma
            target_frac = max(-self.MR_MAX_FRAC, min(self.MR_MAX_FRAC, -self.MR_K * z))
            target = int(target_frac * limit)

        return self._execute_mr(product, od, pos, target, z, bids, asks, best_bid, best_ask)

    # ---- VELVET MR using EMA-of-mid (verbatim from harry_v4) ------------
    def _trade_mr_velvet(self, product: str, od: OrderDepth, pos: int,
                         td: dict) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        best_bid = bids[0]
        best_ask = asks[0]
        if best_ask - best_bid < self.MR_TIGHT_SPREAD_MIN:
            return []
        mid = (best_bid + best_ask) / 2.0

        ema_key = f"ema_{product}"
        var_key = f"var_{product}"
        n_key = f"n_{product}"
        prev_ema = td.get(ema_key, FALLBACK_VELVET_FV)
        prev_var = td.get(var_key, FALLBACK_VELVET_SIGMA ** 2)
        n = td.get(n_key, 0)
        residual = mid - prev_ema
        new_ema = prev_ema + self.EMA_ALPHA * residual
        new_var = prev_var + self.VAR_ALPHA * (residual * residual - prev_var)
        td[ema_key] = new_ema
        td[var_key] = max(1.0, new_var)
        td[n_key] = n + 1
        sigma = max(1.0, new_var) ** 0.5

        limit = LIMITS[product]
        if n + 1 < self.MR_MIN_N or sigma <= 0:
            target = 0
            z = 0.0
        else:
            z = (mid - new_ema) / sigma
            target_frac = max(-self.MR_MAX_FRAC, min(self.MR_MAX_FRAC, -self.MR_K * z))
            target = int(target_frac * limit)

        return self._execute_mr(product, od, pos, target, z, bids, asks, best_bid, best_ask)

    # ---- Shared MR execution (cross-spread + layered passive) -----------
    def _execute_mr(self, product, od, pos, target, z, bids, asks,
                    best_bid, best_ask) -> List[Order]:
        limit = LIMITS[product]
        buy_room = limit - pos
        sell_room = limit + pos
        buy_ordered = 0
        sell_ordered = 0
        orders: List[Order] = []
        diff = target - pos

        if abs(z) >= self.MR_TAKE_Z:
            if diff > 0:
                cap = min(diff, self.MR_TAKE_MAX, buy_room)
                for ap in asks:
                    if cap <= 0:
                        break
                    vol = -od.sell_orders[ap]
                    qty = min(vol, cap)
                    if qty <= 0:
                        continue
                    orders.append(Order(product, ap, qty))
                    buy_ordered += qty
                    cap -= qty
            elif diff < 0:
                cap = min(-diff, self.MR_TAKE_MAX, sell_room)
                for bp in bids:
                    if cap <= 0:
                        break
                    vol = od.buy_orders[bp]
                    qty = min(vol, cap)
                    if qty <= 0:
                        continue
                    orders.append(Order(product, bp, -qty))
                    sell_ordered += qty
                    cap -= qty

        diff_after = target - (pos + buy_ordered - sell_ordered)
        big_side_size = self.MR_MM_LEVEL_SIZE
        small_side_size = max(5, self.MR_MM_LEVEL_SIZE // 3)

        if diff_after > 0:
            buy_size_each = big_side_size
            sell_size_each = small_side_size
        elif diff_after < 0:
            buy_size_each = small_side_size
            sell_size_each = big_side_size
        else:
            buy_size_each = small_side_size
            sell_size_each = small_side_size

        remaining_buy_room = max(0, buy_room - buy_ordered)
        remaining_sell_room = max(0, sell_room - sell_ordered)

        for level in range(self.MR_MM_LEVELS):
            our_bid = best_bid + 1 + level
            our_ask = best_ask - 1 - level
            if our_bid >= our_ask:
                break
            bqty = min(buy_size_each, remaining_buy_room)
            aqty = min(sell_size_each, remaining_sell_room)
            if bqty > 0:
                orders.append(Order(product, our_bid, bqty))
                remaining_buy_room -= bqty
            if aqty > 0:
                orders.append(Order(product, our_ask, -aqty))
                remaining_sell_room -= aqty
        return orders

    # ---- harry_v4 OBI MM (verbatim) -------------------------------------
    def _trade_vev_mm(self, product: str, od: OrderDepth, pos: int) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        best_bid = bids[0]
        best_ask = asks[0]
        if best_ask - best_bid < self.VEV_TIGHT_SPREAD_MIN:
            return []
        our_bid = best_bid + 1
        our_ask = best_ask - 1
        if our_bid >= our_ask:
            return []

        b1_vol = od.buy_orders[best_bid]
        a1_vol = -od.sell_orders[best_ask]
        total = b1_vol + a1_vol
        obi = 0.0 if total == 0 else (b1_vol - a1_vol) / total
        buy_size, sell_size = self._vev_obi_sizes(obi)

        limit = LIMITS[product]
        soft_thresh = int(self.VEV_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos
        buy_qty = min(buy_size, max(0, buy_room))
        sell_qty = min(sell_size, max(0, sell_room))

        if pos >= soft_thresh:
            buy_qty = 0
        elif pos <= -soft_thresh:
            sell_qty = 0

        orders: List[Order] = []
        if buy_qty > 0:
            orders.append(Order(product, our_bid, buy_qty))
        if sell_qty > 0:
            orders.append(Order(product, our_ask, -sell_qty))
        return orders

    def _vev_obi_sizes(self, obi: float) -> Tuple[int, int]:
        abs_obi = abs(obi)
        if abs_obi < self.OBI_SKEW_1:
            return (self.VEV_BASE_SIZE, self.VEV_BASE_SIZE)
        if abs_obi < self.OBI_SKEW_2:
            big, small = self.VEV_SIZES_MILD
        elif abs_obi < self.OBI_SKEW_3:
            big, small = self.VEV_SIZES_STRONG
        else:
            big, small = self.VEV_SIZES_EXTREME
        return (big, small) if obi > 0 else (small, big)

    def _parse_td(self, s: Optional[str]) -> dict:
        if not s:
            return {}
        try:
            return json.loads(s)
        except Exception:
            return {}
