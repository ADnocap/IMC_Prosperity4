"""Wolf2 — wolf strategy, pushed to MAX inventory.

STRATEGY UNCHANGED FROM WOLF — only position sizes scaled up.

Wolf portal sub 475990 (1K-tick UI backtest, day 2): +1,520 total. Per-asset
PnL was POSITIVE on every income leg (HYDROGEL +773 / VELVET +527 / VEV_4000
+134 / VEV_4500 +99). The strategy is right; sizes were too small.

Changes vs wolf (sizes only, no logic changes):
  - HYDROGEL (limit 200): OBI_SIZE_MAX 97->195, K_CONF 1.0->2.0,
    MM_BASE 54->130, REVZ_TAKE 22->80, SOFT_POS_FRAC 0.43->0.85
  - VELVETFRUIT MR (limit 200): MR_K 0.045->0.20, MR_MAX_FRAC 0.49->0.90,
    MM_LEVEL_SIZE 13->60
  - VEV_4000/4500 (limit 300): BASE_MM 37->140, tier sizes 4-6x,
    SOFT_POS_FRAC 0.6->0.85
  - Cross-strike: sizes UNCHANGED at wolf's 30/40/20 — audit found
    diminishing returns on bigger CS sizes (size 100 5300/5400 had similar
    PnL to size 30 5200/5400 in historical replay).
"""
...
CS_PAIRS = [
    # Reverted to wolf's sizes — bigger CS sizes added MC noise (-4,155)
    # without improving portal alpha (already +11k/3d historical at 30 lots).
...
class Trader:
    # === HYDROGEL (porush logic, MAX sizes) ===============================
    HY_OBI_THRESH = 0.11598037104847925
    HY_OBI_SKEW_TICKS = 1
    HY_OBI_SIZE_K_CONF = 2.0              # was 1.0 (porush) - max scaling
    HY_OBI_SIZE_MAX = 195                 # was 97 - 97.5% of 200 limit
    HY_OBI_SIZE_MIN = 15                  # was 5
    HY_MM_BASE_SIZE = 130                 # was 54 - 65% of limit each side passive
    HY_REVZ_WINDOW = 385
    HY_REVZ_THRESHOLD = 2.0
    HY_REVZ_OBI_GATE = 0.08054351512797084
    HY_REVZ_TAKE_SIZE = 80                # was 22 - 4x bigger directional take
    HY_REVZ_HOLD = 446
    HY_SOFT_POS_FRAC = 0.85               # was 0.43 - bleed at 170 (85% of 200)
    HY_TIGHT_SPREAD_MIN = 2

    # === Jordan/stratton MR core - VELVETFRUIT scaled to MAX =============
    EMA_ALPHA = 4.308428675778431e-05
    VAR_ALPHA = 0.03588256506509171
    MR_K = 0.20                           # was 0.045 - 4.4x more directional
    MR_MAX_FRAC = 0.90                    # was 0.49 - allow 90% of limit at extreme z
    MR_MIN_N = 50
    MR_MM_LEVEL_SIZE = 60                 # was 13 - 4.6x bigger passive quotes
    MR_MM_LEVELS = 1
    ...
    # === VEV_4000/4500 OBI MM - MAX sizes ================================
    VEV_BASE_SIZE = 8                     # was 3
    VEV_TIGHT_SPREAD_MIN = 2
    VEV_SOFT_POS_FRAC = 0.85              # was 0.6 - bleed at 255/300 (85%)
    OBI_SKEW_1 = 0.17962297493671575
    OBI_SKEW_2 = 0.4541715247701739
    OBI_SKEW_3 = 0.7103899562397099
    VEV_SIZES_MILD = (90, 25)             # was (22, 8) - 4x big side
    VEV_SIZES_STRONG = (130, 8)           # was (30, 2)
    VEV_SIZES_EXTREME = (180, 10)         # was (40, 3) - 4.5x big side, 60% of limit

    OBI_SKEW_T1_THRESH = 0.08480914085182328
    OBI_SKEW_T2_THRESH = 0.7984188975781722
    OBI_SKEW_T1_TICKS = 0
    OBI_SKEW_T2_TICKS = 2
    BASE_MM_SIZE = 140                    # was 37 - 3.8x penny-jump floor
