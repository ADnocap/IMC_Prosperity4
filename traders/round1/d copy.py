...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # ══════════════════════════════════════════════════════
        # Phase 3: MAKE — penny/join + passive clearing
        # ══════════════════════════════════════════════════════
        ...
        # Inventory-aware quote shift: tighten UNWIND side by 1 tick
        # when position is extended. No capacity split — just move
        # the existing order closer to attract clearing fills.
        # Gentle: max 1 tick shift, only on the side that reduces position.
        if pos_after_take > 25:
            our_ask -= 1  # tighten ask when long
        elif pos_after_take < -25:
            our_bid += 1  # tighten bid when short
        ...
        # Full capacity at single level — no split
        if buy_room > 0:
