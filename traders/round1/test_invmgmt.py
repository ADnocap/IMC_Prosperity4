"""Test: OSMIUM with inventory management - flatten when heavily positioned.
Hypothesis: keeping position near 0 maximizes fill capacity on both sides."""
...
class Trader:
    ...
    def _osmium(self, od, pos, td):
        ...
        # PASSIVE with inventory-aware quote skewing
        ...
        # Inventory management: shift quotes by 1 tick when heavily positioned
        # At pos > 40: move ask closer to FV (easier to sell)
        # At pos < -40: move bid closer to FV (easier to buy)
        if pos > 40:
    ...
    def _wall_bid(bids_s, fv_r):
        ...
        if len(bids_s) >= 2:
            return bids_s[0] if fv_r - bids_s[0] >= 5 else bids_s[1]
    ...
    def _wall_ask(asks_s, fv_r):
        ...
        if len(asks_s) >= 2:
            return asks_s[0] if asks_s[0] - fv_r >= 5 else asks_s[1]
