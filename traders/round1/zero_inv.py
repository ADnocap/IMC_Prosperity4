"""Zero-inventory OSMIUM: aggressive taking + immediate clearing.
The #3-6 cluster all have max DD = 422.455 (= pure PEPPER DD), meaning
their OSMIUM strategy adds ZERO drawdown. This suggests they either:
1. Don't hold OSMIUM positions, or
2. Clear so fast that OSMIUM never causes DD.

Approach: take profitable orders AND immediately clear any position.
Quote passively but with smaller size to limit inventory risk."""
import json
...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # AGGRESSIVE CLEAR: flatten ALL inventory at FV or better
        ...
        # MAKE: penny-jump with CAPPED size to limit inventory
        buy_room = LIMIT - starting_pos - buy_ordered
        sell_room = LIMIT + starting_pos - sell_ordered

        # Cap passive order size to keep inventory manageable
        max_passive = 40  # Don't offer more than 40 per side
