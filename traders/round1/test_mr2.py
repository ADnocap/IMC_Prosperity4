...
class Trader:
    """
    OSMIUM: Standard MM + mean-reversion taking.

    Key insight: DON'T modify passive quotes (that destroys MM edge).
    Instead, use the TAKING phase for mean-reversion:
    - When FV deviates from 10000, take MORE aggressively in the
      reversion direction (even at slight negative edge)
    - This builds inventory that profits from reversion
    - The MM continues to earn spread independently

    The mean reversion is subtle: FV std=4.5 vs random walk std=31.
    FV is bounded ±18 from 10000. Reversion is slow (half-life ~200 ticks)
    but reliable over longer horizons.
    """
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Standard taking: buy asks at ≤ fv_r, sell bids at ≥ fv_r
        # MR enhancement: when deviation is large, widen taking threshold
        # in the reversion direction by 1-2 ticks

        buy_threshold = fv_r   # buy asks at this price or below
        sell_threshold = fv_r  # sell bids at this price or above

        # When FV below mean → want to buy more → raise buy threshold
        if deviation < -3:
            buy_threshold = fv_r + 1
        if deviation < -6:
            buy_threshold = fv_r + 2

        # When FV above mean → want to sell more → lower sell threshold
        if deviation > 3:
            sell_threshold = fv_r - 1
        if deviation > 6:
            sell_threshold = fv_r - 2
        ...
        # ── Phase 2: Standard penny-jump passive (UNCHANGED) ──
