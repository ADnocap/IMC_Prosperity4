"""Parameterized OSMIUM strategy for grid search."""
...
TAKE_EDGE = int(os.environ.get("TAKE_EDGE", "0"))  # min edge for taking (0=current)
QUOTE_OFFSET = int(os.environ.get("QUOTE_OFFSET", "7"))  # passive quote distance from FV
...
class Trader:
    ...
    def _osmium(self, od, pos, td):
        ...
        # TAKING with configurable edge threshold
        take_threshold_buy = fv_r - TAKE_EDGE  # buy asks at this price or below
        take_threshold_sell = fv_r + TAKE_EDGE  # sell bids at this price or above

        for p in asks_s:
            if p > take_threshold_buy:
                break
            v = -od.sell_orders[p]
        ...
        # PASSIVE at configurable offset
        our_bid = fv_r - QUOTE_OFFSET
        our_ask = fv_r + QUOTE_OFFSET

        # When using penny-jump mode (offset=7), find the wall and penny-jump
        if QUOTE_OFFSET == 7:
