"""Tight spread: FV±6 with penny-jump. Tests if avg_fill ~6 is optimal."""
import json
...
EDGE = 6  # Quote at FV±6 instead of FV±7
...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # MAKE: penny-jump with default FV±6
        ...
        for bp in bids:
            if bp <= fv_r - 1:
                dist = fv_r - bp
                if dist <= 2:
                    our_bid = bp      # Join within 2
                elif dist <= EDGE:
                    our_bid = bp + 1  # Penny within edge
                # else: keep default FV-EDGE
                break
