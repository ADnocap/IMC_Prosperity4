...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # OU-driven pull with L2-gap reinforcement.
        center_dist = fv_r - 10000
        l2_dir = 0
        if len(bids) >= 2 and len(asks) >= 2:
            gb = bids[0] - bids[1]
            ga = asks[1] - asks[0]
            if gb == 2 and ga == 3: l2_dir = 1
            elif gb == 3 and ga == 2: l2_dir = -1

        # Lower OU threshold when L2-gap concurs (87% conditional accuracy).
        pull_bid = False
        pull_ask = False
        K_ou = 9
        K_combined = 6
        if center_dist >= K_ou and pos_after > -80:
            pull_bid = True
        elif center_dist >= K_combined and l2_dir == -1 and pos_after > -80:
            pull_bid = True
        if center_dist <= -K_ou and pos_after < 80:
            pull_ask = True
        elif center_dist <= -K_combined and l2_dir == 1 and pos_after < 80:
        ...
        if pull_bid: buy_cap = 0
        if pull_ask: sell_cap = 0
