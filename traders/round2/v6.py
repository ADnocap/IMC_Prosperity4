...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # v6: adaptive spread. Tight (FV±3) when near-neutral to attract flow and
        # cycle frequently; wide (FV±7) when loaded up to extract maximum edge on
        # position unwind. Trade-off: smaller edge × more fills vs big edge × fewer.
        if abs(pos_after) < 20:
            our_bid = min(fv_r - 3 + obi_shift, fv_r - 1)
            our_ask = max(fv_r + 3 + obi_shift, fv_r + 1)
        else:
