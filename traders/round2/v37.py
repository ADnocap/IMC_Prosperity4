...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Take (skip direction-wrong Bot3 takes during strong OU signal)
        ou_dist = fv_r - 10000
        skip_buys = ou_dist >= 9   # expect down, don't buy up
        skip_sells = ou_dist <= -9  # expect up, don't sell short
        if not skip_buys:
            for ap in asks:
                if ap > fv_r - 2:
                    break
