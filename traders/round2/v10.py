...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        default_bid = min(ref_bid + 1 + obi_shift, fv_r - 1)
        default_ask = max(ref_ask - 1 + obi_shift, fv_r + 1)

        # v10: OU-directional MM. Bias quoting to accumulate inventory in the direction
        # of expected mean-reversion. When FV > 10000, we want SHORT inventory (sell
        # more eagerly via tighter ask), and vice versa. Natural MM fills build the
        # position; when FV reverts, position MTM captures the OU edge.
        ...
        if center_dist >= 4 and pos_after > -50:
            our_ask = max(default_ask - 2, fv_r + 3)
            buy_cap = 20
            sell_cap = 50
        elif center_dist <= -4 and pos_after < 50:
            our_bid = min(default_bid + 2, fv_r - 3)
            buy_cap = 50
            sell_cap = 20

        # Inventory safety: force unwind when near hard limit regardless of OU side.
        if pos_after >= 60:
            our_ask = max(default_ask - 2, fv_r + 2)
            buy_cap = min(buy_cap, 15)
            sell_cap = 60
        elif pos_after <= -60:
            our_bid = min(default_bid + 2, fv_r - 2)
            buy_cap = 60
            sell_cap = min(sell_cap, 15)
