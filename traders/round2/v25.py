...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # v25: aggressive Bot2 take at extreme OU dist (profitable only at |dist| >= 15)
        ex_dist = fv_r - 10000
        if ex_dist >= 15 and starting_pos - sell_ordered > -70:
            for bp in bids:
                if bp < fv_r - 8: break
                vol = od.buy_orders[bp]
                can = LIMIT + starting_pos - sell_ordered
                if can <= 0: break
                qty = min(vol, can, 10)  # cap at 10 per tick
                if qty > 0:
                    orders.append(Order(OSMIUM, bp, -qty))
                    sell_ordered += qty
        elif ex_dist <= -15 and starting_pos + buy_ordered < 70:
            for ap in asks:
                if ap > fv_r + 8: break
                vol = -od.sell_orders[ap]
                can = LIMIT - starting_pos - buy_ordered
