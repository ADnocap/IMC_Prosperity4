...
class Trader:
    ...
    # ML price shift: center our quote spread around expected future FV.
    # Keeps flow (no pull); tilts the mid we collect spread around.
    ML_SHIFT_HI = 0.55   # p_up >= => our_bid+=1, our_ask+=1
    ML_SHIFT_LO = 0.45   # p_up <= => our_bid-=1, our_ask-=1
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # ML price shift: tilt the quote mid toward expected future FV.
        center_dist = fv_r - 10000
        if abs(center_dist) < 9:
            if p_up >= self.ML_SHIFT_HI:
                our_bid += 1
                our_ask += 1
            elif p_up <= self.ML_SHIFT_LO:
                our_bid -= 1
                our_ask -= 1
