...
class Trader:
    ...
    # ML skew thresholds. p_up > ML_HI => expect up => buy more, sell less.
    ML_HI = 0.55
    ML_LO = 0.45
    ML_SKEW = 10  # units added to one cap and subtracted from the other
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # ML skew: bias caps toward the predicted direction (soft).
        # Only fires when confident (|p - 0.5| > 0.05) and inventory is light.
        if abs(pos_after) < self.ACO_SOFT_POS:
            if p_up > self.ML_HI:
                buy_cap += self.ML_SKEW
                sell_cap -= self.ML_SKEW
            elif p_up < self.ML_LO:
                buy_cap -= self.ML_SKEW
                sell_cap += self.ML_SKEW
