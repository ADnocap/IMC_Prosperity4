...
class Trader:
    ...
    IPR_OBI_BULL = 0.45
    IPR_OBI_BEAR = -0.45
    IPR_BEAR_BID_CLIP = 35
    IPR_BEAR_ASK_POS = 45
    IPR_BEAR_ASK_QTY = 8
    IPR_BEAR_ASK_OFFSET = 7
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        dense_book = spread <= self.ACO_DENSE_SPREAD_MAX and (bv1 + av1) >= self.ACO_DENSE_L1_MIN
        take_edge = 1 if dense_book else 2
        ...
        buy_cap = 42 if dense_book else 40
        sell_cap = 42 if dense_book else 40
        ...
        # Tighten quotes on heavy inventory — only when the adaptive signal is live.
        if mode != 0:
    ...
    def _trade_ipr(self, od: OrderDepth, position: int, td: dict):
        ...
        bv1 = od.buy_orders[bids_sorted[0]] if bids_sorted else 0
        av1 = -od.sell_orders[asks_sorted[0]] if asks_sorted else 0
        total_l1 = bv1 + av1
        obi = (bv1 - av1) / total_l1 if total_l1 > 0 else 0.0

        buy_vol_limit = 15
        if obi >= self.IPR_OBI_BULL:
            buy_vol_limit = 20
        elif obi <= self.IPR_OBI_BEAR:
            buy_vol_limit = 10

        # Sweep asks with an OBI-adaptive volume cap:
        # bearish tape -> avoid deep catches; bullish tape -> allow extra aggression.
        for ask_price in asks_sorted:
            if remaining <= 0:
                break
            vol = -od.sell_orders[ask_price]
            if vol > buy_vol_limit:
                continue
        ...
        if remaining > 0:
            if bids_sorted:
                ...
                our_bid = fv_r - 6
            passive_qty = remaining
            if obi <= self.IPR_OBI_BEAR:
                passive_qty = min(passive_qty, self.IPR_BEAR_BID_CLIP)
            if passive_qty > 0:
                orders.append(Order(PEPPER, our_bid, passive_qty))

        sell_room = LIMIT + position
        if obi <= self.IPR_OBI_BEAR and position >= self.IPR_BEAR_ASK_POS and sell_room > 0:
            early_unload = min(self.IPR_BEAR_ASK_QTY, sell_room)
            if early_unload > 0:
                orders.append(Order(PEPPER, fv_r + self.IPR_BEAR_ASK_OFFSET, -early_unload))
                sell_room -= early_unload
