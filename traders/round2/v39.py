...
# Logistic regression weights trained on all 6 days (R1 -2..0, R2 -1..1).
# Target: sign(FV_{t+50} - FV_t). CV mean AUC 0.65, acc 0.61 (std 0.02).
# Features in order below; score = BIAS + sum(W_i * x_i), p_up = sigmoid(score).
ML_BIAS = 0.041350
ML_W_CENTER_DIST = -0.103787
ML_W_RET1        = -0.190590
ML_W_RET5        = -0.079549
ML_W_RET20       = -0.019519
ML_W_BID_GAP     = +0.001261
ML_W_ASK_GAP     = +0.011402
ML_W_L2_GAP_SIGN = +0.133479
ML_W_SPREAD      = +0.007132
ML_W_OBI         = -0.053976
ML_W_LOG_L1_VOL  = -0.033919
...
class Trader:
    ...
    # ML-driven one-sided pulls. Size skews don't convert (elastic qty caps at 10),
    # but pulling the wrong-side quote does — we give up one side of flow when the
    # model is confidently directional, same mechanism as the OU pull.
    ML_PULL_HI = 0.60   # p_up >= => pull ASK (expect up, don't sell here)
    ML_PULL_LO = 0.40   # p_up <= => pull BID (expect down, don't buy here)
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # ML pull: when model is strongly directional, pull the wrong-side quote.
        # Inventory-guarded so we don't stack pulls that breach limits.
        if p_up >= self.ML_PULL_HI and pos_after < 70:
            sell_cap = 0
        elif p_up <= self.ML_PULL_LO and pos_after > -70:
            buy_cap = 0

        # OU-directional one-sided passive (keep — richer signal than single p threshold).
        center_dist = fv_r - 10000
