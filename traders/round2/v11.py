...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # v11: max-size quotes. Post full 80 per side. Sim has elastic-qty cap at 10
        # so this won't help in MC — but on portal, if takers can be larger, we capture
        # more flow. Also makes our book THICKER, which may attract more counterparties.
        buy_cap = 80
        sell_cap = 80

        if signal > 0 and abs(pos_after) < self.ACO_SOFT_POS:
            buy_cap, sell_cap = 80, 70
        elif signal < 0 and abs(pos_after) < self.ACO_SOFT_POS:
            buy_cap, sell_cap = 70, 80
