...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # v5: multi-tier quoting. Split buy_room between a tight inner bid (edge 3)
        # and the normal outer bid at FV±7. Inner bids serve as "magnets" that may
        # attract portal takers who'd otherwise bypass us. Worst case: inner + outer
        # both fill → combined ≤ buy_room (already within limit).
        inner_bid = min(fv_r - 3, fv_r - 1)
        inner_ask = max(fv_r + 3, fv_r + 1)
        inner_size = 10
        outer_buy = max(0, buy_room - inner_size)
        outer_sell = max(0, sell_room - inner_size)

        if buy_room > 0:
            # Tight inner bid first (best price for takers)
            if buy_room >= inner_size:
                orders.append(Order(OSMIUM, inner_bid, inner_size))
                if outer_buy > 0:
                    orders.append(Order(OSMIUM, our_bid, outer_buy))
            else:
                orders.append(Order(OSMIUM, inner_bid, buy_room))
        if sell_room > 0:
            if sell_room >= inner_size:
                orders.append(Order(OSMIUM, inner_ask, -inner_size))
                if outer_sell > 0:
                    orders.append(Order(OSMIUM, our_ask, -outer_sell))
            else:
                orders.append(Order(OSMIUM, inner_ask, -sell_room))
