...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # OU + last-move combined.
        prev_fv_r = td.get("last_fv_r", fv_r)
        last_move = td.get("last_move", 0)
        if fv_r > prev_fv_r:
            last_move = 1
        elif fv_r < prev_fv_r:
            last_move = -1
        td["last_fv_r"] = fv_r
        td["last_move"] = last_move

        center_dist = fv_r - 10000
        pull_bid = False
        pull_ask = False
        if center_dist >= 9 and pos_after > -80:
            pull_bid = True
        elif center_dist >= 5 and last_move >= 1 and pos_after > -80:
            pull_bid = True
        if center_dist <= -9 and pos_after < 80:
            pull_ask = True
        elif center_dist <= -5 and last_move <= -1 and pos_after < 80:
