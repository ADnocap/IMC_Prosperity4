...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # v16: OU-directional one-sided passive trading.
        # When FV is far from 10000, only quote the side that builds OU-aligned
        # inventory. This captures MTM during the reversion instead of wasting bid
        # fills at high FV that would need to be unwound at lower FV.
        center_dist = fv_r - 10000
        if center_dist >= 14 and pos_after > -70:
            # FV high, expect down → only post ask (accumulate short)
            buy_cap = 0
        elif center_dist <= -14 and pos_after < 70:
            # FV low, expect up → only post bid (accumulate long)
            sell_cap = 0

        # Inventory safety: if we've heavily built in any direction, resume symmetric
        # MM to facilitate unwind (or reduce risk if the signal flips on us).
        if pos_after >= 60:
