...
class Trader:
    ...
    # ML gate on OU: only pull at extreme center_dist when ML *also* predicts reversion.
    # Veto threshold must be strict — weak-residual model, false positives hurt.
    ML_CONFIRM = 0.50   # pull buy at center_dist>=9 requires p_up <= ML_CONFIRM (and symmetric)
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # OU pull, gated by ML residual confirmation. Only pull when the residual
        # model *also* predicts the same direction of reversion — ML says
        # "yes, nothing in the book contradicts the OU expectation".
        center_dist = fv_r - 10000
        if center_dist >= 9 and pos_after > -70 and p_up <= self.ML_CONFIRM:
            buy_cap = 0
        elif center_dist <= -9 and pos_after < 70 and p_up >= (1.0 - self.ML_CONFIRM):
            sell_cap = 0
