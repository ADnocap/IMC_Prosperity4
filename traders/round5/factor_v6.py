"""Round 5 factor_v6 — per-asset adaptive EMA half-life + RW disabled.
...
Fix:
  ...
  - Set HL=0 for RW assets to skip the per-asset MR signal entirely
    (no mean to revert to → EMA-MR is just noise on a random walk)

Snackpack pair K signal kept (this is the ONLY basket trade with valid theory).
Directional close-the-gap execution (no MM tail) — the v1 pattern.
"""
...
HL_PER_ASSET: Tuple[int, ...] = (
    1108, 0, 0, 2198, 1671, 1671, 1671, 0, 1671, 0,
    2521, 2198, 1671, 1270, 1108, 1916, 1000, 1457, 0, 1500,
    1108, 0, 1000, 1000, 0, 3315, 1270, 0, 1000, 1457,
    1671, 1000, 1671, 0, 1671, 0, 0, 0, 1457, 0,
    2198, 1457, 0, 1916, 2891, 1000, 1500, 1000, 1000, 1000,
)
# Precompute alpha = 1 - 2^(-1/HL); 0 → 0 (no update)
