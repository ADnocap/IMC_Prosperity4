...
# ---------------------------------------------------------------------------
# Pebble layer (port of Pebble_fucker_v1)
# ---------------------------------------------------------------------------
...
def _pebble_step(state: TradingState, _td: dict) -> Dict[str, List[Order]]:
    ...
    for p in PEBBLES:
        ...
        if od is None or not od.buy_orders or not od.sell_orders:
            return out  # bail on incomplete book — basket constraint unreliable
    ...
    if residual > PEBBLE_RESIDUAL_THRESHOLD:
        allow_bid, allow_ask = False, True
    elif residual < -PEBBLE_RESIDUAL_THRESHOLD:
        allow_bid, allow_ask = True, False
    else:
