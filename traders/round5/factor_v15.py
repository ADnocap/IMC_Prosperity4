"""Round 5 factor_v15 — v12 + heavy-tail enable.

Per-asset PnL analysis on v12 (100-session MC) showed ROBOT_DISHES (HL=1000)
earns +19,422 — the single biggest contributor. Mechanism: kurt=25 (heavy tails)
+ fast-OU half-life 319 + enabled EMA-MR signal → catches the snap-backs.

ROBOT_IRONING (kurt=17.8, v12 mean +296) and OXYGEN_SHAKE_EVENING_BREATH
(kurt=17.2, v12 mean +1921) have similar heavy tails but are currently
SIGNAL-DISABLED (HL=0, MM-only).

v15 enables both at HL=1000 (matching ROBOT_DISHES) to test if the kurtosis-
driven MR signal generalizes. If it does, expect a mean lift of ~3-7K.
"""
...
HL_PER_ASSET: Tuple[int, ...] = (
    ...
    1108, 0, 1000, 1000, 1000, 0, 0, 0, 1000, 1457,  # idx 24 ROBOT_IRONING: 0->1000
    0, 1000, 1671, 0, 1671, 0, 0, 0, 1457, 0,
    0, 1000, 0, 1916, 0, 1000, 1500, 1000, 1000, 1000,  # idx 41 OXY_EVENING_BREATH: 0->1000
)
