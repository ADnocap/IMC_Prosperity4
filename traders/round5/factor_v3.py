"""Round 5 factor_v3 — adaptive sizing on top of v2's 2-sided MM.

v2 left buy/sell budget on the table (BIG_QTY=7 capped bids even when target=+10
with pos=0, leaving 3 units idle). v3 sizes the directional side to use the FULL
remaining position-limit budget toward the target, plus a SPREAD_QTY tail for
spread harvest on the opposite side.

Sizing rule (per asset, after computing target):
    diff = target - pos
    if diff > 0:
        bid_qty = min(buy_room, diff + SPREAD_QTY)     # close the gap + a bit more
        ask_qty = min(sell_room, SPREAD_QTY)            # spread tail
    elif diff < 0:
        bid_qty = min(buy_room,  SPREAD_QTY)
        ask_qty = min(sell_room, -diff + SPREAD_QTY)
    else:
        bid_qty = ask_qty = min(SPREAD_QTY, room)       # balanced 2-sided MM

The +SPREAD_QTY on the directional side helps when bot pulses sweep more than
needed; the spread-tail on the opposite side catches contrarian pulses for
spread harvest. Both sides obey worst-case-fill position-limit math.

Also widened the alpha signal slightly: TARGET_FRAC = 0.9 (was 0.6) so that
the trader actually uses the position limits when the signal is strong.
"""
...
MR_K_TRIPLET = 1.0
OBI_K = 0.3
TARGET_FRAC = 0.9         # was 0.6 in v2 — actually use the limits when alpha is strong
SPREAD_QTY = 4            # 2-sided spread tail size
TIGHT_SPREAD_MIN = 2
...
class Trader:
    def run(self, state: TradingState):
        ...
        for sym in ASSETS:
            ...
            # Adaptive sizing — use full directional budget
            if diff > 0:
                bid_q = min(buy_room, diff + SPREAD_QTY)
                ask_q = min(sell_room, SPREAD_QTY)
            elif diff < 0:
                bid_q = min(buy_room, SPREAD_QTY)
                ask_q = min(sell_room, -diff + SPREAD_QTY)
            else:
                bid_q = min(buy_room, SPREAD_QTY)
                ask_q = min(sell_room, SPREAD_QTY)
