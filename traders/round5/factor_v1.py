...
worst-case math (Σ buys ≤ 10 - position, Σ sells ≤ 10 + position).

State persistence: traderData = JSON dict {symbol: fair_value} so EMAs survive
across ticks.

Parameters (v1, conservative defaults):
  HL_TICKS  = 1500   # EMA half-life — close to median OU half-life
  KAPPA     = 4.0    # alpha-to-position scale (max alpha 3 → ±12 → clipped to ±10)
  ALPHA_CAP = 3.0    # cap absolute alpha at 3σ
...
# B_EXPOSURE[k][i] = sigma_i * V_corr[k, i] — exposure of asset i to factor k
...
B_EXPOSURE_FULL: Tuple[Tuple[float, ...], ...] = (
    # PC1 — snackpack triplet (PIS / STRAW / RASP)
    (-0.04242, -0.05476, -0.16199, -0.04178, -0.07688, +0.07810, -0.03242,
     ...
     +4.61541),
    # PC2 — pebble basket (XL vs M/L/XS/S)
    ...
    (+0.03997, +0.11265, +0.06184, +0.10426, +0.04659, +0.01816, +0.01435,
     ...
     -0.08923),
    # PC4 — weak broad mixed factor (borderline stable, kept commented for reference)
...
N_FACTORS_USED = 3        # use first 3 stable PCs (drop PC4)


# ---------------------------------------------------------------------------
# Linear-algebra helpers (pure Python — keeps the file dependency-light)
# ---------------------------------------------------------------------------
