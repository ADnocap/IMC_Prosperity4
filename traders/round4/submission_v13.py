"""V13 prototype — V10 + EMA_ALPHA sweep on stratton MR.

V10 isolated MR_K=0.080 as the only size knob that mattered (HY_MM_BASE_SIZE,
BASE_MM_SIZE, size tiers all flat). But it never tested the EMA_ALPHA on
the stratton MR fair-value EMA. V13 reads EMA_ALPHA (and VAR_ALPHA) from
PROSPERITY_PARAMS env var so we can sweep without recompiling.

Defaults reproduce V10 byte-for-byte (PROSPERITY_PARAMS unset).

Decision: ship best v13 config only if total >= V10 + 200 AND holdout >= V10.
...
class Trader:
    ...
    def __init__(self):
        import os as _os
        raw = _os.environ.get("PROSPERITY_PARAMS")
        if raw:
            try:
                p = json.loads(raw)
            except Exception:
                p = {}
            for k in ("EMA_ALPHA", "VAR_ALPHA", "MR_K", "MR_MAX_FRAC"):
                if k in p:
                    setattr(self, k, p[k])
