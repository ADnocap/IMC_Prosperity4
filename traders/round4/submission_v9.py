"""V9 prototype — V2 + Carter-style quote-around-BS-fair MM for IV-scalp idle phase.

The smile_a_sweep audit (analysis/round4/smile_a_sweep.md) explicitly flagged:
  "Future PnL beyond V2 needs a *different signal* during the idle phase
   (e.g. counterparty-conditioned MM or a longer-horizon mean reversion on
   mid - bs_fair_static), not a refinement of the same EMA."

V9 implements that "mean-reversion on mid - bs_fair_static" — when V2's
IV-scalp activity gate is closed (94% of ticks), instead of falling
through to stratton MR (which quotes around EMA mid, mechanically the
same as bot mid), v9 routes the voucher to a Carter-style quote-around-
BS-fair handler:

   bid = max(floor(bs_fair), best_bid + 1)
   ask = min(ceil(bs_fair) + V9_BSFAIR_HALF_SPREAD, best_ask - 1)

This anchors our quotes to the model fair, not the bot fair. If bot
quotes drift even slightly off BS-fair (which happens often outside
the IV-scalp activity window — the EMA smile tracks slowly), our
quotes capture the resulting mid-revert flow as a passive MM edge,
without paying spread.

Reference: tmp/p3_research/carter_trader.py:933-952 (Carter quotes at
int(theo) and int(theo)+1 with full position-limit size).
...
# === V9: Carter-style quote-around-BS-fair MM ============================
# When IV-scalp is idle on a voucher, route to BS-fair MM instead of
# stratton MR fallback. Set to False to recover V2 byte-for-byte.
V9_BSFAIR_MM_ENABLED = True
# Vouchers that get the BS-fair quote handler. Excluded: VEV_4000/4500
# (deep ITM, BS-fair is too close to the intrinsic for tight quoting),
# VEV_6000/6500 (pinned at 0/1).
V9_BSFAIR_VOUCHERS = ("VEV_5000", "VEV_5100", "VEV_5200", "VEV_5300",
                       "VEV_5400", "VEV_5500")
...
class Trader:
    ...
    # === V9: BS-fair quote-around params ===================================
    V9_BSFAIR_HALF_SPREAD = 0     # ask = ceil(theo) (1-tick book-spread OK)
    V9_BSFAIR_BID_SIZE = 12
    V9_BSFAIR_ASK_SIZE = 12
    V9_BSFAIR_SOFT_POS_FRAC = 0.5
    V9_BSFAIR_DEV_THR = 1.0       # only fire when |mid - fair| > this
    V9_BSFAIR_BIAS_BOOST = 2      # extra size on the favoured side
    ...
    # === V9: BS-fair-biased MM handler ====================================
    def _trade_bsfair_mm(self, sym: str, od: OrderDepth, pos: int,
                          ctx: Dict[str, dict]) -> List[Order]:
        """Penny-jump MM with size biased by mid - BS-fair sign.
        - mid > fair (voucher rich) → push the sell side, slim the buy side.
        - mid < fair (voucher cheap) → push the buy side, slim the sell side.
        Only activates when |mid - fair| > V9_BSFAIR_DEV_THR (skip noise);
        otherwise return empty so V2's MR/MM fallback runs.
        """
        info = ctx.get(sym)
        if info is None:
            return []
        ...
        mid = info["mid"]
        dev = mid - fair
        if abs(dev) < self.V9_BSFAIR_DEV_THR:
            return []   # let stratton MR / vev_mm handle the noise band
        ...
        if best_ask - best_bid < 1:
            return []
        our_bid = best_bid + 1 if best_ask - best_bid >= 2 else best_bid
        our_ask = best_ask - 1 if best_ask - best_bid >= 2 else best_ask
        if our_bid >= our_ask:
            return []

        # Direction-biased size.
        if dev > 0:    # rich → sell-biased
            bqty = self.V9_BSFAIR_BID_SIZE
            aqty = self.V9_BSFAIR_ASK_SIZE + self.V9_BSFAIR_BIAS_BOOST
        else:          # cheap → buy-biased
            bqty = self.V9_BSFAIR_BID_SIZE + self.V9_BSFAIR_BIAS_BOOST
            aqty = self.V9_BSFAIR_ASK_SIZE

        limit = LIMITS[sym]
        soft_thresh = int(self.V9_BSFAIR_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos
        bqty = min(bqty, max(0, buy_room))
        aqty = min(aqty, max(0, sell_room))
        if pos >= soft_thresh:
            bqty = 0
        ...
        for product in state.order_depths:
            ...
            if product == HYDROGEL:
                ...
                if fired:
                    ...
                    elif (V9_BSFAIR_MM_ENABLED
                          and product in V9_BSFAIR_VOUCHERS):
                        # Priority 2.5: V9 BS-fair quote-around fallback.
                        result[product] = self._trade_bsfair_mm(
                            product, od, pos, ctx)
                    else:
