"""V8 prototype — V2 with EMA → double-EMA (TMA-like) on all 4 sites.

Same logic as V2 but every EMA is applied twice with the same alpha,
producing a smoother / more lagging filter equivalent to a triangular
moving average:
  TMA-like(x, alpha) = EMA(EMA(x, alpha), alpha)

Sites swapped:
  1. Smile coefficient online refit (`smile_a`).
  2. Per-voucher `mean_dev` (THEO_NORM_WINDOW EMA).
  3. Per-voucher `switch_mean` (IV_SCALPING_WINDOW EMA, the activity gate).
  4. Stratton MR fair-value EMA (`_update_ema`, EMA_ALPHA, VAR_ALPHA).

Each new state key is `<old_key>_2` (the second EMA stage). The double-
EMA value replaces the single EMA wherever the V2 logic referenced it
downstream.

Decision rule for shipping: same overfit-aware audit as v7. If
double-EMA improves on BOTH train (D1+D2) AND holdout (D3) over V2 by
a meaningful margin, ship v8. Else, keep V2.
...
class Trader:
    ...
    SMILE_B = 0.033704
    SMILE_C = 0.089775
    # V8: alphas DOUBLED / windows HALVED so TMA lag matches V2 EMA lag.
    # (Double-EMA(alpha) lag ≈ 2 × single-EMA(alpha) lag.)
    SMILE_A_ALPHA = 0.0104          # 2x V2's 0.0052
    ...
    THEO_NORM_WINDOW = 50           # V8: half V2's 100 (TMA lag-match)
    IV_SCALPING_WINDOW = 100        # V8: half V2's 200 (TMA lag-match)
    ...
    EMA_ALPHA = 8.616857351556862e-05  # V8: 2x V2's 4.31e-5 (TMA lag-match)
    VAR_ALPHA = 0.07176513013018342    # V8: 2x V2's 0.0359 (TMA lag-match)
    ...
    def _build_market_ctx(self, state: TradingState, td: dict
        ...
        prev_a_2 = td.get("smile_a_2", self.SMILE_A_INIT)
        if observed_ivs:
            residuals = []
            for K, iv_obs in observed_ivs.items():
                iv_pred = self._smile_iv(K, S, T_years, prev_a_2)
                residuals.append(iv_obs - iv_pred)
            avg_res = sum(residuals) / len(residuals)
            # Double-EMA (TMA-like): apply SMILE_A_ALPHA twice.
            new_a = prev_a + self.SMILE_A_ALPHA * avg_res
            new_a_2 = prev_a_2 + self.SMILE_A_ALPHA * (new_a - prev_a_2)
            new_a = max(0.05, min(3.0, new_a))
            new_a_2 = max(0.05, min(3.0, new_a_2))
            td["smile_a"] = new_a
            td["smile_a_2"] = new_a_2
            smile_a = new_a_2
        else:
            smile_a = prev_a_2
        ...
        for sym in SCALP_VOUCHERS:
            ...
            mean_key2 = f"theo_mean_{sym}_2"
            sw_key = f"switch_mean_{sym}"
            sw_key2 = f"switch_mean_{sym}_2"
            prev_mean = td.get(mean_key, dev)
            prev_mean2 = td.get(mean_key2, dev)
            alpha_m = 2.0 / (self.THEO_NORM_WINDOW + 1)
            new_mean = alpha_m * dev + (1 - alpha_m) * prev_mean
            new_mean2 = alpha_m * new_mean + (1 - alpha_m) * prev_mean2
            td[mean_key] = new_mean
            td[mean_key2] = new_mean2
            prev_sw = td.get(sw_key, 0.0)
            prev_sw2 = td.get(sw_key2, 0.0)
            alpha_s = 2.0 / (self.IV_SCALPING_WINDOW + 1)
            sw_input = abs(dev - new_mean2)
            new_sw = alpha_s * sw_input + (1 - alpha_s) * prev_sw
            new_sw2 = alpha_s * new_sw + (1 - alpha_s) * prev_sw2
            td[sw_key] = new_sw
            td[sw_key2] = new_sw2
            ctx[sym] = {
                "S": S, "T": T_years, "fair": fair, "dev": dev,
                "mean_dev": new_mean2, "switch_mean": new_sw2,
    ...
    def _update_ema(self, td: dict, product: str, mid: float
        ...
        # V8: double-EMA (TMA-like) on both FV and variance.
        ema_key = f"ema_{product}"
        ema2_key = f"ema_{product}_2"
        var_key = f"var_{product}"
        var2_key = f"var_{product}_2"
        n_key = f"n_{product}"
        prev_ema = td.get(ema_key)
        prev_ema2 = td.get(ema2_key)
        prev_var = td.get(var_key, FALLBACK_SIGMA.get(product, 30.0) ** 2)
        prev_var2 = td.get(var2_key, FALLBACK_SIGMA.get(product, 30.0) ** 2)
        n = td.get(n_key, 0)
        if prev_ema is None:
            prev_ema = FALLBACK_FV.get(product, mid)
        if prev_ema2 is None:
            prev_ema2 = prev_ema
        residual = mid - prev_ema2
        new_ema = prev_ema + self.EMA_ALPHA * (mid - prev_ema)
        new_ema2 = prev_ema2 + self.EMA_ALPHA * (new_ema - prev_ema2)
        new_var = prev_var + self.VAR_ALPHA * (residual * residual - prev_var)
        new_var2 = prev_var2 + self.VAR_ALPHA * (new_var - prev_var2)
        new_n = n + 1
        td[ema_key] = new_ema
        td[ema2_key] = new_ema2
        td[var_key] = max(1.0, new_var)
        td[var2_key] = max(1.0, new_var2)
        td[n_key] = new_n
        sigma = max(1.0, new_var2) ** 0.5
        return new_ema2, sigma, new_n
