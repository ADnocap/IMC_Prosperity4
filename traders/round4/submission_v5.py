"""DO-NOT-SHIP — V5 prototype: R3 OBI handlers (KILLED 2026-04-27).

Tested vs V2 +29,934: total +27,640 (Δ -2,294).
  VELVET _trade_velvet_obi   : -2,326 vs stratton MR (handler structurally
                               worse — naked OBI MM has no inventory-pull
                               protection, so D3 drift accumulates wrong-way
                               position).
  VEV_4000 _trade_deep_itm   : +26 (flat — V2's penny-jump already captures
                               this via Mark14↔Mark38 fills; per
                               marks_d_fallback diagnostic).
  VEV_4500                    : 0  (no-trade strike).

The R3 final_audit numbers (+2-4k/day on VELVET, +1-2.5k/day on VEV_4000)
were projections based on theoretical OBI-tier capture rate (~5-30%), never
tested on full V2 stack. On R4 data the projections do not materialise.

V2 baseline retained (KEEP V2).

# === V5 disabled below ================================================
# Original V2 docstring follows.

Adds (per analysis/round3/final_audit.md secs 1-2):
  1. NEW _trade_velvet_obi for VELVETFRUIT:
       - OBI tier confidence sizing on |OBI|>=VEL_OBI_THRESH (0.30 default)
       - Asymmetric SIZE only (no quote-tick skew, spread is 5 ticks)
       - Replaces stratton _trade_mr for VELVET.
       Estimated R3 uplift: +2-4k/day.
  2. NEW _trade_deep_itm_obi for VEV_4000 + VEV_4500:
       - Same OBI tier logic, larger size (V4K_OBI_SIZE_MAX=120)
       - Penny-jump quote with single-tick OBI skew
       - Falls through to existing _trade_vev_mm when |OBI|<threshold.
       Estimated R3 uplift: +1-2.5k/day on VEV_4000.

Sources: R3 analysis/round3/final_audit.md, FINDINGS_v2.md, signal_decay.md.
Both signals were never shipped because R3 search-2 (stratton) optimised
under MR_K=0.045 only and never re-ran the param search after the audit.
The R4 V2 inherited stratton verbatim; these handlers are orthogonal.
...
# === New OBI handler toggles (V5) =======================================
# Both disabled by default after kill verdict; flip to reproduce.
VELVET_OBI_HANDLER = False
DEEP_ITM_OBI_HANDLER = False
...
class Trader:
    ...
    # === V5: VELVET OBI handler params (per R3 final_audit sec 1) =========
    # VELVET spread is 5 ticks (vs HYDROGEL 16); no quote-tick skew, only
    # asymmetric size on |OBI|>=threshold.
    VEL_OBI_THRESH = 0.30
    VEL_OBI_SIZE_MAX = 80          # confidence-scaled cap
    VEL_OBI_SIZE_MIN = 8
    VEL_OBI_SIZE_K_CONF = 0.7
    VEL_MM_BASE_SIZE = 18
    VEL_SOFT_POS_FRAC = 0.65
    VEL_TIGHT_SPREAD_MIN = 2

    # === V5: VEV_4000/4500 deep-OBI handler params (R3 final_audit sec 2) =
    V4K_OBI_THRESH = 0.30
    V4K_OBI_SKEW_TICKS = 1
    V4K_OBI_SIZE_MAX = 120
    V4K_OBI_SIZE_MIN = 5
    V4K_OBI_SIZE_K_CONF = 0.8
    V4K_MM_BASE_SIZE = 20
    V4K_SOFT_POS_FRAC = 0.6
    V4K_TIGHT_SPREAD_MIN = 4
    ...
    # === V5: VELVET OBI handler ===========================================
    def _trade_velvet_obi(self, od: OrderDepth, pos: int) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        best_bid, best_ask = bids[0], asks[0]
        if best_ask - best_bid < self.VEL_TIGHT_SPREAD_MIN:
            return []
        ...
        soft_thresh = int(self.VEL_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos

        if abs_obi >= self.VEL_OBI_THRESH:
            conf_size = max(self.VEL_OBI_SIZE_MIN,
                            min(self.VEL_OBI_SIZE_MAX,
                                int(math.ceil(limit * abs_obi
                                                * self.VEL_OBI_SIZE_K_CONF))))
            if obi > 0:
                buy_size = conf_size
                sell_size = self.VEL_MM_BASE_SIZE
            else:
                buy_size = self.VEL_MM_BASE_SIZE
                sell_size = conf_size
        else:
            buy_size = sell_size = self.VEL_MM_BASE_SIZE

        # No quote-tick skew (spread too tight). Penny-jump only.
        our_bid = best_bid + 1
        our_ask = best_ask - 1
        if our_bid >= our_ask:
            return []
        bqty = min(buy_size, max(0, buy_room))
        aqty = min(sell_size, max(0, sell_room))
        if pos >= soft_thresh:
            bqty = 0
    ...
    # === V5: deep-ITM OBI handler (VEV_4000 / VEV_4500) ===================
    def _trade_deep_itm_obi(self, product: str, od: OrderDepth, pos: int
                              ) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        best_bid, best_ask = bids[0], asks[0]
        if best_ask - best_bid < self.V4K_TIGHT_SPREAD_MIN:
            return []
        b1_vol = od.buy_orders[best_bid]
        a1_vol = -od.sell_orders[best_ask]
        total = b1_vol + a1_vol
        ...
        # Below threshold -> fall through to V2 _trade_vev_mm.
        if abs_obi < self.V4K_OBI_THRESH:
            return self._trade_vev_mm(product, od, pos)

        limit = LIMITS[product]
        soft_thresh = int(self.V4K_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos
        conf_size = max(self.V4K_OBI_SIZE_MIN,
                        min(self.V4K_OBI_SIZE_MAX,
                            int(math.ceil(limit * abs_obi
                                            * self.V4K_OBI_SIZE_K_CONF))))
        if obi > 0:
            buy_size = conf_size
            sell_size = self.V4K_MM_BASE_SIZE
            our_bid = best_bid + 1
            our_ask = best_ask + max(0, self.V4K_OBI_SKEW_TICKS) - 1
        else:
            buy_size = self.V4K_MM_BASE_SIZE
            sell_size = conf_size
            our_bid = best_bid - max(0, self.V4K_OBI_SKEW_TICKS) + 1
            our_ask = best_ask - 1
        if our_bid >= our_ask:
            our_bid = best_bid + 1
            our_ask = best_ask - 1
    ...
    def run(self, state: TradingState
        ...
        for product in state.order_depths:
            ...
            if product == HYDROGEL:
                ...
                if VELVET_OBI_HANDLER and product == VELVETFRUIT:
                    result[product] = self._trade_velvet_obi(od, pos)
                else:
                    result[product] = self._trade_mr(product, od, pos, td)
            elif product in VEV_MM_ASSETS:
                if DEEP_ITM_OBI_HANDLER:
                    result[product] = self._trade_deep_itm_obi(product, od, pos)
                else:
                    result[product] = self._trade_vev_mm(product, od, pos)
            else:
                result[product] = []
