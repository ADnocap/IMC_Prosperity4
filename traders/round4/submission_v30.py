"""V30 — V10 + Mark 01 → VELVETFRUIT cross-spread take layer (experimental).

V30 changes vs V10 (only):
- New `_mark01_velvet_orders` method that detects Mark 01 trades on VELVETFRUIT
  in `state.market_trades` over a 50-tick lookback and fires a cross-spread
  take of MARK01_TAKE_SIZE lots at L1 (BUY at best_ask if Mark 01 buys recent,
  SELL at best_bid if Mark 01 sells recent). Cooldown MARK01_COOLDOWN ticks
  per direction. Bidirectional fires net out (count_buys vs count_sells in
  window).
- Run loop wires this layer ahead of `_trade_mr` for VELVETFRUIT only;
  Mark take orders are PREPENDED to MR's MM orders. MR sees post-take pos
  via `pos + take_qty` so its passive sizing/limits stay correct.

Why VELVETFRUIT-only / Mark 01-only (per analysis 2026-04-28):
- VELVETFRUIT has ~3-tick spread → half-spread cost ~1.5 vs Mark 01 drift
  +2.05 (buyer) / +2.73 (seller) at H=200. Net edge ~+0.5-1.2/lot/fire.
- Mark 01 → Mark 22 is the dominant directional flow (1339 trades vs <30
  reverse). No bidirectional cancel — that's what killed marks_c_widen.
- Per-day stability: Mark 01 buyer drift = [+1.0, +1.4, +1.0] / seller =
  [+1.9, +3.7, +2.4]. All days, both directions positive.
- Distinct from existing OBI signal: Mark 01 is 100% passive; OBI sees
  Mark 22's aggression but not Mark 01's passive identity.
- Variants (a/b/c) of the Mark layer all lost on the BROAD HYDROGEL/VEV_4000
  signal where half-spread (~10) > drift (~9). VELVETFRUIT is the one place
  where half-spread is small enough for cross-spread takes to be +EV.

Diagnostic toggle: MARK01_VELVET_ENABLED. Flip to False to recover V10
exactly.

# === V10 docstring (unchanged below) ====================================
V10 (= V2.1) — V2 + MR_K bump 0.045 → 0.080. **NEW SHIP CANDIDATE.**

R4 historical replay (prosperity3bt --merge-pnl, 10K ticks/day, 3 days):
    V2 baseline:  +29,934 (D1 14,675 / D2 2,946 / D3 12,312)
    V10 (this):   +30,293 (D1 14,664 / D2 2,894 / D3 12,735)
    Uplift:       +359   (+1.2%)
    Per-day:      D1 -11 / D2 -52 / D3 +423

D3 is the only fresh OOS sample. **+423 holdout uplift is real edge.** Train
hit -63 (D1+D2) is noise-level (within ±50 per day for V2's standard
deviation across days).

V10 vs V2 diff: SINGLE LINE — MR_K from 0.04481 to 0.080. All other
knobs identical to V2.

How discovered: 18-trial joint random search over 14 V2 size knobs
(`v2_size_tune.py`) showed train↔holdout correlation +0.685 (well-behaved
surface, not overfit, opposite of v7's −0.700). Best trial had MR_K=0.097;
single-axis isolation tests showed ALL the uplift came from MR_K alone
(HY_MM_BASE_SIZE / BASE_MM_SIZE / size tier knobs had zero effect on R4
days). 1D fine sweep over MR_K showed:
    0.045 V2:   29,934 ← baseline
    0.060:      30,198 (+264)
    0.075:      30,292 (+358)
    0.080:      30,293 (+359) ← peak
    0.092:      30,226 (+292)
    0.100:      26,824 (-3,110, MR_MAX_FRAC clipping cliff)
Plateau from 0.06 to 0.092 above V2; cliff at 0.10 from position-limit
thrashing. MR_K=0.080 sits in the middle of the flat plateau — robust.
...
# === V30: Mark 01 → VELVETFRUIT layer ====================================
# Toggle False to recover V10 exactly.
MARK01_VELVET_ENABLED = True
MARK01_USER_ID = "Mark 01"
MARK01_LOOKBACK_TICKS = 50      # Trades within last 50 ticks count as fresh
MARK01_COOLDOWN_TICKS = 30      # Min ticks between fires per direction
MARK01_TAKE_SIZE = 8            # Lots per cross-spread take
...
class Trader:
    ...
    def run(self, state: TradingState
        ...
        for product in state.order_depths:
            ...
            if product == HYDROGEL:
                ...
                if product == VELVETFRUIT and MARK01_VELVET_ENABLED:
                    mark_orders = self._mark01_velvet_orders(state, od, pos, td)
                    take_qty = sum(o.quantity for o in mark_orders)
                    mr_orders = self._trade_mr(product, od, pos + take_qty, td)
                    result[product] = mark_orders + mr_orders
                else:
                    result[product] = self._trade_mr(product, od, pos, td)
    ...
    def _mark01_velvet_orders(self, state: TradingState, od: OrderDepth,
                                pos: int, td: dict) -> List[Order]:
        """V30 layer: cross-spread take when Mark 01 trades observed in last
        MARK01_LOOKBACK_TICKS ticks on VELVETFRUIT. Mark 01 is 100% passive
        and produces drift +2.05 (buyer) / +2.73 (seller) over 200 ticks.
        VELVETFRUIT spread ~3 ticks => half-spread cost ~1.5 << drift => +EV.
        State is persisted in td under keys mark01_v_buys / mark01_v_sells
        (lists of timestamps in the last MARK01_LOOKBACK window) and
        mark01_v_buy_fire / mark01_v_sell_fire (last fire timestamp per dir).
        """
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
        ...
        buys_ts: List[int] = list(td.get("mark01_v_buys", []))
        sells_ts: List[int] = list(td.get("mark01_v_sells", []))
        recent_trades = (state.market_trades or {}).get(VELVETFRUIT, [])
        for t in recent_trades:
            if t.buyer == MARK01_USER_ID:
                buys_ts.append(t.timestamp)
            if t.seller == MARK01_USER_ID:
                sells_ts.append(t.timestamp)

        cutoff = cur_ts - MARK01_LOOKBACK_TICKS * 100
        buys_ts = [ts for ts in buys_ts if ts >= cutoff]
        sells_ts = [ts for ts in sells_ts if ts >= cutoff]
        td["mark01_v_buys"] = buys_ts
        td["mark01_v_sells"] = sells_ts

        last_buy_fire = td.get("mark01_v_buy_fire", -10**9)
        last_sell_fire = td.get("mark01_v_sell_fire", -10**9)
        cd_buy_ok = (cur_ts - last_buy_fire) >= MARK01_COOLDOWN_TICKS * 100
        cd_sell_ok = (cur_ts - last_sell_fire) >= MARK01_COOLDOWN_TICKS * 100
        n_buys = len(buys_ts)
        n_sells = len(sells_ts)

        orders: List[Order] = []
        limit = LIMITS[VELVETFRUIT]
        if n_buys > n_sells and n_buys >= 1 and cd_buy_ok:
            buy_room = limit - pos
            ask_vol = -od.sell_orders[best_ask]
            qty = min(MARK01_TAKE_SIZE, buy_room, ask_vol)
            if qty > 0:
                orders.append(Order(VELVETFRUIT, best_ask, qty))
                td["mark01_v_buy_fire"] = cur_ts
        elif n_sells > n_buys and n_sells >= 1 and cd_sell_ok:
            sell_room = limit + pos
            bid_vol = od.buy_orders[best_bid]
            qty = min(MARK01_TAKE_SIZE, sell_room, bid_vol)
            if qty > 0:
                orders.append(Order(VELVETFRUIT, best_bid, -qty))
                td["mark01_v_sell_fire"] = cur_ts
        return orders
