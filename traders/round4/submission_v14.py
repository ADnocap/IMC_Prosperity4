"""V14 prototype — V10 + dedicated VEV_4500 BS-fair MM. **DO-NOT-SHIP.**

V14 routes VEV_4500 through a new handler quoting at floor(bs_fair)-HALF /
ceil(bs_fair)+HALF (inside the 16-tick book) instead of stratton's
penny-jump at best±1.

Verdict: NOT-VALIDATABLE in replay. Replay PnL = +30,293 (= V10) because
VEV_4500 has only **3 historical trades total across 3 days × 10K ticks**
(D1: 1, D2: 0, D3: 2). No flow → no fills regardless of where we quote.
Handler IS firing (~4000 fires/day, prints confirmed) and quotes are
well-inside book (e.g. 744/749 vs book 738/754). Replay gives zero
information on whether this would help in live portal.

R3 portal final showed +1,888 on VEV_4500 for the same trader, which
hints at non-zero live edge. But without replay validation we cannot
ship per the audit-gated process. Keep this file for diagnostic /
portal A-B test if user chooses to risk a live sub.

Default: V14_VEV4500_ENABLED = False so the file is byte-for-byte V10.

# === Original V10 docstring ============================================
...
class Trader:
    ...
    # === V14 VEV_4500 BS-fair MM (NOT VALIDATABLE — see docstring) ========
    V14_VEV4500_ENABLED = False  # default off; replay can't validate
    V14_HALF = 2          # quote at floor(bs_fair) - 2 / ceil(bs_fair) + 2
    V14_BID_SIZE = 8
    V14_ASK_SIZE = 8
    V14_DEV_GATE = 30.0   # skip if |mid - bs_fair| > this (book is wild)
    V14_SOFT_POS_FRAC = 0.5
    ...
    def run(self, state: TradingState
        ...
        for product in state.order_depths:
            ...
            if product == HYDROGEL:
                ...
                if (self.V14_VEV4500_ENABLED and product == VEV_4500):
                    orders = self._trade_vev4500_bsfair(product, od, pos, ctx)
                    if orders is None:
                        orders = self._trade_vev_mm(product, od, pos)
    ...
    def _trade_vev4500_bsfair(self, product: str, od: OrderDepth, pos: int,
                                ctx: Dict[str, dict]) -> Optional[List[Order]]:
        """V14 dedicated VEV_4500 handler: quote around BS-fair instead of
        penny-jumping inside a 16-tick book. Returns None to fall through
        to _trade_vev_mm if the gate fails."""
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return None
        ...
        if best_ask - best_bid < 4:
            # If spread is tight, penny-jump is fine
            return None
        S = ctx.get("__S__", {}).get("S", FALLBACK_FV[VELVETFRUIT])
        T_years = ctx.get("__S__", {}).get("T", self._T_years(0))
        smile_a = ctx.get("__S__", {}).get("smile_a", self.SMILE_A_INIT)
        K = STRIKES[product]
        bs_fair = self._smile_fair(K, S, T_years, smile_a)
        mid = (best_bid + best_ask) / 2.0
        if abs(mid - bs_fair) > self.V14_DEV_GATE:
            return None
        our_bid = max(int(math.floor(bs_fair)) - self.V14_HALF, best_bid + 1)
        our_ask = min(int(math.ceil(bs_fair)) + self.V14_HALF, best_ask - 1)
        if our_bid >= our_ask:
            return None
        limit = LIMITS[product]
        soft_thresh = int(self.V14_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos
        bqty = min(self.V14_BID_SIZE, max(0, buy_room))
        aqty = min(self.V14_ASK_SIZE, max(0, sell_room))
        if pos >= soft_thresh:
            bqty = 0
        elif pos <= -soft_thresh:
            aqty = 0
