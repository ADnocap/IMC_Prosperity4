...
class Trader:
    ...
    def _trade_velvet_optimp_take(self, od: OrderDepth, pos: int,
        ...
        td["optimp_ctr"] = ctr
        # DIAG: log drift every 500 ticks regardless of fire status.
        if ctr % 500 == 0:
            print(f"OPTIMPDRIFT ctr={ctr} drift={drift:.3f} pos={pos}")
        ...
        if drift >= self.OPTIMP_THR and buy_room > 0:
            ...
            if orders:
                ...
                tot_qty = sum(o.quantity for o in orders)
                px = orders[0].price
                print(f"OPTIMPFIRE ctr={ctr} drift={drift:.3f} dir=+1 "
                      f"qty={tot_qty} px={px} pos={pos} bid={bids[0]} ask={asks[0]}")
