"""DO-NOT-SHIP — V6 prototype: V2 + voucher-delta hedge (KILLED 2026-04-27).

Tested vs V2 +29,934 R4 replay (--merge-pnl):
  V6a — full hedge (DELTA_HEDGE_MIN_ABS=8, FRAC=1.0):
        Total +25,555 (Δ -4,379). VELVET D3 -939 vs V2 +2,759.
  V6b — partial hedge (MIN_ABS=50, DEADBAND=50, FRAC=0.5):
        Total +26,369 (Δ -3,565). Same D3 collapse mechanism.

Structural reason: V2's stratton _trade_mr on VELVET implicitly
*responds* to underlying drift — long-bias when mid below EMA fair, short
when above. On D3 (drift day), stratton's natural MR hedge already covers
most of the voucher-induced delta exposure. Adding explicit hedge bias
to the MR target double-counts the exposure → over-shorts → loses
+3.5k of D3 stratton MR alpha.

Same D3-disaster pattern as v3b (Mark-flow additive bias). Any external
target_offset on _trade_mr collides with stratton's native drift response.

Eric (P2 +145k with delta hedge) didn't have stratton MR — his trader
WAS the hedge layer, no underlying-MR alpha to compete with. We can't
add hedging on top of stratton without losing stratton's edge first.

V2 ships.

New mechanism not tried on R4: after V2 places all voucher orders, sum
aggregate voucher delta (using BS delta from each strike's smile IV).
Pass -aggregate_delta as target_offset to _trade_mr(VELVETFRUIT, ...) so
stratton's MR target gets ADDITIVELY biased by the hedge requirement.

Eric (2nd P2) used this and made +145k. R3 delta_hedged_options.md tested
hedging on STANDALONE always-on short-vol (small +166-1,033/day) but
never on V2's actual dynamic IV-scalp positions, which are larger and
more concentrated during fires.

Hypothesis: V2's IV-scalp opens ±100-300 voucher positions during the
~6% of active ticks. Aggregate delta during fires is significant
(50-150 underlying-equivalent). Hedging with VELVET reduces variance
and eliminates wrong-way drift losses. Spread cost (5 ticks per leg)
amortizes over multi-tick holds.
...
# === V6: delta-hedge layer ==============================================
DELTA_HEDGE_ENABLED = False  # KILL verdict — flip to reproduce.
# Vouchers included in the delta sum. Limited to strikes with reliable BS
# deltas (skip 6000/6500 which have ~0 vega). 4000/4500 are deep ITM
# delta ~1; ATM strikes have delta 0.05-0.6 from smile.
DELTA_HEDGE_VOUCHERS = ("VEV_4000", "VEV_4500", "VEV_5000", "VEV_5100",
                        "VEV_5200", "VEV_5300", "VEV_5400", "VEV_5500")
# Don't hedge tiny exposures (saves spread cost on noise).
DELTA_HEDGE_MIN_ABS = 50
# Only hedge the excess above this — leaves stratton MR room to capture
# its drift alpha on small positions.
DELTA_HEDGE_DEADBAND = 50
# Hedge only this fraction of computed delta (1.0 = full, 0.5 = half).
DELTA_HEDGE_FRAC = 0.5
...
class Trader:
    ...
    # === V6: aggregate delta hedge target ===================================
    def _compute_voucher_delta(self, state: TradingState, ctx: Dict[str, dict],
                                 result: Dict[str, List[Order]]) -> float:
        """Sum projected post-trade voucher delta across DELTA_HEDGE_VOUCHERS.
        Uses BS delta from each strike's smile IV in ctx, falling back to
        deep-ITM (delta=1) for VEV_4000/4500 when not in ctx."""
        S = ctx.get("__S__", {}).get("S")
        T = ctx.get("__S__", {}).get("T")
        smile_a = ctx.get("__S__", {}).get("smile_a", self.SMILE_A_INIT)
        if S is None or S <= 0 or T is None or T <= 0:
            return 0.0
        total = 0.0
        for sym in DELTA_HEDGE_VOUCHERS:
            cur_pos = state.position.get(sym, 0)
            new_qty = sum(o.quantity for o in result.get(sym, []))
            proj = cur_pos + new_qty
            if proj == 0:
                continue
            K = STRIKES[sym]
            info = ctx.get(sym)
    ...
    def run(self, state: TradingState
        ...
        # Two-phase loop so VELVETFRUIT processes last and can hedge the
        # aggregate voucher delta produced by phase 1.
        velvet_od = None
        velvet_pos = 0
        ...
        # Phase 2: VELVETFRUIT with delta-hedge offset.
        if velvet_od is not None:
            offset = 0
            if DELTA_HEDGE_ENABLED:
                voucher_delta = self._compute_voucher_delta(state, ctx, result)
                if abs(voucher_delta) >= DELTA_HEDGE_MIN_ABS:
                    # Excess-only hedge: ignore the deadband; partial frac.
                    excess = (abs(voucher_delta) - DELTA_HEDGE_DEADBAND)
                    if excess > 0:
                        sgn = 1 if voucher_delta > 0 else -1
                        offset = -int(round(sgn * excess * DELTA_HEDGE_FRAC))
            result[VELVETFRUIT] = self._trade_mr(
                VELVETFRUIT, velvet_od, velvet_pos, td, target_offset=offset)
