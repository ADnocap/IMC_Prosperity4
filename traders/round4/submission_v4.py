"""DO-NOT-SHIP — V4 prototype: V2 + L3 static theta carry (KILLED 2026-04-27).

5 variants tested vs V2 +29,934 R4 replay (--merge-pnl):
  A: 5300/5500 vert ±100 -> +28,416 (Δ -1,518). VEV_5500 long leg loses.
  B: 5300/5400 vert ±50  -> +27,650 (Δ -2,284). 5400 long leg loses.
  C: short 5300 only -50 -> +28,888 (Δ -1,046).
  D: short 5300 only -25 -> +28,994 (Δ   -940).  (LEFT IN PLACE)
  E: short 5300 only -10 -> +28,866 (Δ -1,068).

Best variant (D) still loses 940 vs V2. R3 cross_strike audit's theta
estimates were based on R3 mid-to-mid; on R4 the 5500/5400 long legs
don't have positive carry (mid was already at-or-below BS-fair on those
strikes), so adding them strictly costs spread without compensating
theta. Single-leg short-5300 is the only positive variant and still
small-negative net.
...
# === L3 — static theta-carry vertical ===================================
# Sell VEV_5300 (richer leg per cross_strike z=+0.36), buy VEV_5500
# (the cheap-side anchor). Hold all session. Disables IV-scalp/MR on these
# two strikes so the static target survives the per-tick gate-closed
# flatten code in _iv_scalp_orders.
L3_ENABLED = False  # Disabled by default after kill verdict; flip to reproduce.
# Variant A: 5300/5500 vert at ±100 -> +28,416 (V2 -1,518)
# Variant B: 5300/5400 vert at ±50 -> +27,650 (V2 -2,284)
# Variant C: short 5300 alone (R3 audit z=+0.36 rich), small size
L3_CARRY_VOUCHERS = ("VEV_5300",)
L3_TARGETS = {"VEV_5300": -10}
...
class Trader:
    ...
    # === V4 — L3 static theta carry =======================================
    L3_MAX_PER_TICK = 12     # per-strike build/unwind cap
    ...
    # === L3 — static theta-carry executor =================================
    def _trade_l3_carry(self, product: str, od: OrderDepth, pos: int,
                         target: int) -> List[Order]:
        """Build toward target via penny-jump on the closing side; gentle
        IOC take only if spread is 1 tick (i.e., free crossing). Sized
        small per tick (L3_MAX_PER_TICK) so we don't ladder past the
        target on burst fills."""
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        ...
        # Free take only if spread == 1 (crossing costs zero edge).
        spread = best_ask - best_bid
        take_cap = self.L3_MAX_PER_TICK if spread == 1 else 0

        if diff > 0:
            cap = min(diff, take_cap, buy_room)
            for ap in asks:
                if cap <= 0:
                    break
                vol = -od.sell_orders[ap]
            ...
            cap = min(-diff, take_cap, sell_room)
            for bp in bids:
                if cap <= 0:
                    break
                vol = od.buy_orders[bp]
        ...
        # Passive penny-jump on the side that closes the gap.
        our_bid = best_bid + 1
        our_ask = best_ask - 1
        if our_bid < our_ask:
            taken_buy = sum(o.quantity for o in orders if o.quantity > 0)
    ...
    def run(self, state: TradingState
        ...
        for product in state.order_depths:
            ...
            # L3 — static theta carry: takes priority on its 2 strikes.
            if L3_ENABLED and product in L3_CARRY_VOUCHERS:
                tgt = L3_TARGETS[product]
                result[product] = self._trade_l3_carry(product, od, pos, tgt)
                continue
            if product == HYDROGEL:
