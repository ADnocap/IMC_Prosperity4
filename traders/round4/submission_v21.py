"""V21 — V10 + lowered IV_SCALPING_THR (1.0865 → 0.7) to fight plateau.

Plateau analysis showed activity gate `switch_mean < IV_SCALPING_THR` shuts
off opens during quiet windows (D2 ts 300K-1M: trade rate dropped from
124/100K to 8-22/100K). Walking the book (v20) failed: realized cash drops
exactly as the avg fill price worsens at L2/L3. So instead try the
direct lever: lower the gate.

V1 used IV_SCALPING_THR=0.7 and made +27,444. V2 raised to 1.0865 -> +29,934.
This v21 tries reverting just that one knob (keeps all other V2-tuned
params + V10's MR_K=0.080).
...
class Trader:
    ...
    IV_SCALPING_THR = 0.7           # V21 (V2 was 1.0865) - lower gate, more fires
