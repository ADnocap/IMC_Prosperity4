...
# === V2 size-knob joint-search tunable contract =========================
# These are the V2 SIZE params that were never jointly tuned post-V2 IV-scalp
# refinement. Hypothesis: VEV_BASE_SIZE=3 is tiny vs limit 300; MR_MM_LEVEL_SIZE=13
# under-utilizes VELVET limit 200; HYDROGEL base could grow.
_TUNABLE_KEYS = (
    "VEV_BASE_SIZE",
    "MR_MM_LEVEL_SIZE",
    "HY_MM_BASE_SIZE",
    ...
    "HY_SOFT_POS_FRAC",
    "VEV_SIZES_MILD_BIG",
    "VEV_SIZES_MILD_SMALL",
    "VEV_SIZES_STRONG_BIG",
    "VEV_SIZES_STRONG_SMALL",
    "VEV_SIZES_EXTREME_BIG",
    "VEV_SIZES_EXTREME_SMALL",
)
...
def _load_param_overrides() -> Dict[str, float]:
    ...
    if not isinstance(data, dict):
        return {}
    return {k: data[k] for k in _TUNABLE_KEYS if k in data}
...
class Trader:
    ...
    def __init__(self):
        ov = _load_param_overrides()
        # Scalars
        scalar_keys = (
            "VEV_BASE_SIZE", "MR_MM_LEVEL_SIZE", "HY_MM_BASE_SIZE",
            "BASE_MM_SIZE", "MR_K", "MR_MAX_FRAC",
            "VEV_SOFT_POS_FRAC", "HY_SOFT_POS_FRAC",
        )
        for k in scalar_keys:
            if k in ov:
                cur = getattr(self, k)
                if isinstance(cur, int) and not isinstance(cur, bool):
                    setattr(self, k, int(ov[k]))
                else:
                    setattr(self, k, float(ov[k]))
        # Tuple sizes — assemble from BIG/SMALL pairs.
        if "VEV_SIZES_MILD_BIG" in ov or "VEV_SIZES_MILD_SMALL" in ov:
            big = int(ov.get("VEV_SIZES_MILD_BIG", self.VEV_SIZES_MILD[0]))
            small = int(ov.get("VEV_SIZES_MILD_SMALL", self.VEV_SIZES_MILD[1]))
            self.VEV_SIZES_MILD = (big, small)
        if "VEV_SIZES_STRONG_BIG" in ov or "VEV_SIZES_STRONG_SMALL" in ov:
            big = int(ov.get("VEV_SIZES_STRONG_BIG", self.VEV_SIZES_STRONG[0]))
            small = int(ov.get("VEV_SIZES_STRONG_SMALL", self.VEV_SIZES_STRONG[1]))
            self.VEV_SIZES_STRONG = (big, small)
        if "VEV_SIZES_EXTREME_BIG" in ov or "VEV_SIZES_EXTREME_SMALL" in ov:
            big = int(ov.get("VEV_SIZES_EXTREME_BIG", self.VEV_SIZES_EXTREME[0]))
            small = int(ov.get("VEV_SIZES_EXTREME_SMALL", self.VEV_SIZES_EXTREME[1]))
            self.VEV_SIZES_EXTREME = (big, small)
