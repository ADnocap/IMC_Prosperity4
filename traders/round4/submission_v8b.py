"""V8b prototype — V2 with TMA only on IV-scalp mean_dev + switch_mean.

Surgical TMA test: replace ONLY the two IV-scalp EMAs (the hot path) with
double-EMA-same-alpha. Stratton MR FV / smile_a stay single-EMA.

Windows halved to keep TMA lag matched to V2's EMA lag:
  THEO_NORM_WINDOW: 100 → 50
  IV_SCALPING_WINDOW: 200 → 100
...
class Trader:
    ...
    THEO_NORM_WINDOW = 50           # V8b: half V2's 100 (TMA lag-match)
    IV_SCALPING_WINDOW = 100        # V8b: half V2's 200 (TMA lag-match)
    ...
    def _build_market_ctx(self, state: TradingState, td: dict
        ...
        for sym in SCALP_VOUCHERS:
            ...
            new_sw = alpha_s * abs(dev - new_mean2) + (1 - alpha_s) * prev_sw
