"""V22 — V10 + passive resting orders at the IV-scalp trigger price.

Plateau cause = activity gate going dormant. v21 (lower gate) regressed.
v20 (walk book) regressed (worse fill prices).

This v22 tries: when the activity gate is ON (switch_mean >= IV_SCALPING_THR)
but the take trigger isn't met yet, post passive resting orders at the
exact price where the take WOULD fire. If the market drifts to that price
within the tick, we get filled at the same price the take logic would have
hit. This adds trades without taking worse fill prices.

  passive_ask = ceil(fair + thr + mean_dev)   (sell side - same trigger as
                                                "(best_bid - fair) - mean_dev >= thr")
  passive_bid = floor(fair - thr + mean_dev)  (buy side, mirror)

Sized at PASSIVE_SCALP_SIZE per side per voucher.
Only posts when current best_bid/best_ask doesn't already cross the trigger
(otherwise the take logic fires).
...
class Trader:
    ...
    PASSIVE_SCALP_ENABLED = True    # V22 - resting orders at trigger price
    PASSIVE_SCALP_SIZE = 8          # V22 - per-side passive size
    ...
    def _iv_scalp_orders(self, sym: str, od: OrderDepth, pos: int,
        ...
        # V22 passive layer: post resting orders at the exact trigger price.
        # Only fires when the take logic didn't already use the inventory budget.
        # If a counterparty walks to the trigger price during the tick, we get
        # filled at the same expected-edge price as the take logic.
        if self.PASSIVE_SCALP_ENABLED:
            ...
            # Sell trigger:  (p - fair) - mean_dev >= thr  ->  p >= fair + thr + mean_dev
            passive_ask = int(math.ceil(fair + thr + mean_dev))
            passive_bid = int(math.floor(fair - thr + mean_dev))
            psize = self.PASSIVE_SCALP_SIZE
            # Only post above current best_ask (don't cross), and only if it's
            # better than just resting at best_ask - 1
            if rem_sell > 0 and passive_ask > best_bid and passive_ask <= best_ask:
                # passive_ask <= best_ask means current book already crosses our
                # trigger - skip (take logic would have fired)
                pass
            elif rem_sell > 0 and passive_ask > best_ask:
                qty = min(psize, rem_sell)
                if qty > 0:
                    orders.append(Order(sym, passive_ask, -qty))
                    fired = True
            if rem_buy > 0 and passive_bid < best_ask and passive_bid >= best_bid:
                pass
            elif rem_buy > 0 and passive_bid < best_bid:
                qty = min(psize, rem_buy)
                if qty > 0:
                    orders.append(Order(sym, passive_bid, qty))
                    fired = True
        return orders, fired
