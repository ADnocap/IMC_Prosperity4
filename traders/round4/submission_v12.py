"""V12 prototype — V10 (MR_K=0.080) + Carter BS-fair MM for IV-scalp idle.

V9 was V2 + BS-fair MM and lost -152 vs V2. V10 then shipped V2 + MR_K bump
(0.045 -> 0.080) for +359 vs V2 (mostly D3 +423). V12 retests the BS-fair
overlay on top of V10 to see if the faster VELVET MR target makes the
voucher BS-fair handler viable (different fallback inventory state could
make BS-fair quotes hit a different population).

V12 = V10 + V9_BSFAIR_MM_ENABLED. Single-line param diff vs V10.
   bid = max(floor(bs_fair), best_bid + 1)
   ask = min(ceil(bs_fair) + V9_BSFAIR_HALF_SPREAD, best_ask - 1)

Decision rule: ship V12 only if total >= V10 + 200 AND holdout (D3) >= V10.
Otherwise keep V10. (V9 vs V2: -65/-36/-49, all 3 days slightly negative.
With V10's MR_K change, behavior on VELVET D3 is different — could shift.)
...
class Trader:
    ...
    MR_K = 0.080  # V10 ship value (V2 was 0.04481152690538941)
