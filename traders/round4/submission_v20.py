"""V20 — V10 + walk-the-book on IV-scalp opens.

V10 IV-scalp logic only sold at best_bid / bought at best_ask, capped by
top-of-book counterparty volume. Trade-size analysis showed we sweep 100%
of L1 (median 8-19 lots, max 32) but the SCALP_MAX_PER_TICK=35 cap is
NEVER hit (0% of trades). Binding constraint = available L1 volume.

V20 walks deeper into the book on opens while the per-level deviation
still meets `thr`. At each price level p on the bid side:
    sell_dev_p = (p - fair) - mean_dev
walks down (descending bids) until sell_dev_p < thr or we hit the
SCALP_MAX_PER_TICK / sell_room cap. Same on buy side (ascending asks).

Naturally limits walking depth: levels with no edge are skipped.
Close-back unchanged (still L1 only - unwinds don't need to chase).
...
class Trader:
    ...
    SCALP_WALK_BOOK = True          # V20 - walk deeper levels while edge holds
    SCALP_WALK_MAX_LEVELS = 3       # V20 - hard cap on book depth (CSV has L1-L3)
    ...
    def _iv_scalp_orders(self, sym: str, od: OrderDepth, pos: int,
        ...
        if sell_dev >= thr and sell_room > 0:
            cap = min(self.SCALP_MAX_PER_TICK, sell_room)
            if self.SCALP_WALK_BOOK:
                levels_used = 0
                for bp in bids:
                    if cap <= 0 or levels_used >= self.SCALP_WALK_MAX_LEVELS:
                        break
                    if (bp - fair) - mean_dev < thr:
                        break
                ...
                qty = min(cap, od.buy_orders[best_bid])
                if qty > 0:
                    orders.append(Order(sym, best_bid, -qty))
                    fired = True
        elif buy_dev <= -thr and buy_room > 0:
            cap = min(self.SCALP_MAX_PER_TICK, buy_room)
            if self.SCALP_WALK_BOOK:
                levels_used = 0
                for ap in asks:
                    if cap <= 0 or levels_used >= self.SCALP_WALK_MAX_LEVELS:
                        break
                    if (ap - fair) - mean_dev > -thr:
                        break
                ...
                qty = min(cap, -od.sell_orders[best_ask])
                if qty > 0:
                    orders.append(Order(sym, best_ask, qty))
                    fired = True
