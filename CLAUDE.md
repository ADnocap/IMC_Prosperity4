...
Active R4 submission file: `traders/round4/submission.py` — **stratton baseline + Timo IV-deviation scalping** on the 6 ATM-area vouchers (VEV_5000-5500). Uses the audit-validated BS smile (`analysis/round4/bachelier_vs_bs.md`) with online refit of the level coefficient. CS spread MR layer DISABLED (cost -37k in replay; revisit separately). HYDROGEL routed through stratton's OBI MM handler (porush handler returned 0 PnL in replay). Counterparty Mark IDs NOT yet used — that's the next layer. R4 historical replay (prosperity3bt --merge-pnl): **+27,444** (D1 14,961 / D2 582 / D3 11,901), vs stratton baseline +20,954. MC --quick: mean +7,858 / std 5,509 (lower than stratton's 10,768 / 6,815 — MC under-evaluates IV-scalping by construction since voucher FVs are independent Brownians). The shipped baseline copy of stratton is preserved at `traders/round4/stratton.py`.

Per-Mark profiling lives in `calibration/marks/`. The R4 strategy thesis: figure out which Marks are aggressive takers vs passive MMs, and which signal direction. Then size more aggressively against directional Marks and avoid being adversely selected by the smart ones.
