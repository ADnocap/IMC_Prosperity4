"""pete_hegseth_v7 - v3 + Path B: scale into Mark 38 own_trades fills."""
...
# === Path B: Mark 38 fade via own_trades, applied as QUOTE SKEW ======
# Mark 38 is an INFORMED WRONG-WAY aggressor (agg_pct=100, drift -7 to -11
# ticks over 200t). When Mark 38 lifts our ask, mid will DROP further -
# we want to keep accumulating short PASSIVELY (no spread crossing cost,
# which would consume most of the drift edge given HYDROGEL's 16-tick
# spread). We do this by skewing our existing OBI MM quote sizes for
# M38_LEAN_TICKS after each Mark 38 fill.
M38_FADE_PRODUCTS = (HYDROGEL, VEV_4000)
M38_LEAN_TICKS = 100          # how long the size lean persists per fill
M38_LEAN_BOOST = 1.4          # multiplier on the favored side's quote size
M38_LEAN_DAMP = 0.7           # multiplier on the disfavored side
...
class Trader:
    ...
    def run(self, state: TradingState
        ...
        for product in state.order_depths:
            ...
            # ===== Refresh Mark 38 lean from this tick's own_trades =====
            if product in M38_FADE_PRODUCTS:
                self._update_mark38_lean(state, product, td, ts)
            ...
            if product in SMILE_VOUCHERS:
                ...
                result[product] = self._trade_mm(product, od, pos, td=td, ts=ts)
            else:
                result[product] = []  # VEV_6000/6500 dead
        return result, 0, json.dumps(td)

    # ===== Mark 38 fade lean (Path B, no spread crossing) =================
    def _update_mark38_lean(self, state: TradingState, product: str,
                              td: dict, ts: int) -> None:
        """Scan THIS tick's own_trades. If Mark 38 was a counterparty,
        write a lean direction + expiry into td. The lean is read by
        _trade_mm to skew quote sizes (no spread crossing cost)."""
        own = (state.own_trades or {}).get(product, []) or []
        for t in own:
            if t.buyer == "Mark 38":
                # Mark 38 lifted our ask; we sold. Mid will drop. Lean SHORT.
                td[f"m38_lean_dir_{product}"] = -1
                td[f"m38_lean_until_{product}"] = ts + M38_LEAN_TICKS * 100
                return
            if t.seller == "Mark 38":
                # Mark 38 hit our bid; we bought. Mid will rise. Lean LONG.
                td[f"m38_lean_dir_{product}"] = +1
                td[f"m38_lean_until_{product}"] = ts + M38_LEAN_TICKS * 100
                return

    def _mark38_lean(self, product: str, td: dict, ts: int) -> int:
        """Returns +1 (lean LONG), -1 (lean SHORT), or 0 (no lean active)."""
        until = td.get(f"m38_lean_until_{product}", -1)
        if ts > until:
            return 0
        return td.get(f"m38_lean_dir_{product}", 0)
    ...
    def _trade_mm(self, product: str, od: OrderDepth, pos: int,
                    td: Optional[dict] = None, ts: int = 0) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        ...
        # Apply Mark 38 lean (Path B): boost the favored side, dampen the
        # other. Active for M38_LEAN_TICKS after a Mark 38 fill.
        if td is not None and product in M38_FADE_PRODUCTS:
            lean = self._mark38_lean(product, td, ts)
            if lean > 0:        # lean LONG: boost bid, dampen ask
                buy_size = int(buy_size * M38_LEAN_BOOST)
                sell_size = max(1, int(sell_size * M38_LEAN_DAMP))
            elif lean < 0:      # lean SHORT: boost ask, dampen bid
                sell_size = int(sell_size * M38_LEAN_BOOST)
                buy_size = max(1, int(buy_size * M38_LEAN_DAMP))
