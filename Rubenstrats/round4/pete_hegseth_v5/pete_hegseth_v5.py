"""pete_hegseth_v5 - v3 + Mark 14 shadow on HYDROGEL / VEV_4000."""
...
# === Mark 14 shadow ====================================================
# Mark 14 is an informed PASSIVE MM (agg_pct=0.0) on HYDROGEL_PACK and
# VEV_4000. When Mark 14 is recorded as buyer at price X, X was their
# resting BID. As seller at X, X was their resting ASK. We quote 1 tick
# INSIDE their inferred levels to get filled before them on flow they
# already considered fair-priced — earning the spread instead of paying it.
SHADOW_PRODUCTS = (HYDROGEL, VEV_4000)
SHADOW_MARK = "Mark 14"
SHADOW_LOOKBACK_TICKS = 50      # window to infer Mark 14's recent levels
SHADOW_QUOTE_SIZE = 15          # lots per side
SHADOW_SOFT_POS_FRAC = 0.6      # cut off the side that grows pos past this
...
class Trader:
    ...
    def run(self, state: TradingState
        ...
        # Maintain Mark 14 trade log for shadow quoting on HYDROGEL/VEV_4000.
        ...
        for product in state.order_depths:
            ...
            elif product in SHADOW_PRODUCTS:
                # Try Mark 14 shadow first; fall back to plain OBI MM if no
                # recent Mark 14 trades to infer their levels.
                shadow_orders = self._shadow_quotes(product, od, pos, td, ts)
                if shadow_orders:
                    result[product] = shadow_orders
                else:
    ...
    def _update_mark14_log(self, state: TradingState, td: dict) -> None:
        """Record Mark 14's recent trades on SHADOW_PRODUCTS as bid/ask
        observations (Mark 14 = passive MM, so trade price ~ their resting
        level on the side they were standing on)."""
        ...
        for sym, trades in (state.market_trades or {}).items():
            ...
            for t in trades:
                ...
                if buyer == SHADOW_MARK:
                    ...
                    if key not in seen:
                        log.append({"sym": sym, "ts": int(t.timestamp),
    ...
    def _recent_m14_levels(self, product: str, td: dict, cur_ts: int
        ...
        """Median of Mark 14's recent (lookback-window) bid and ask prints."""
        log: List[Dict] = td.get("m14_trades", [])
        cutoff = cur_ts - SHADOW_LOOKBACK_TICKS * 100
        bids: List[int] = []
        asks: List[int] = []
        for e in log:
            if e["sym"] != product or e["ts"] < cutoff:
                continue
            (bids if e["side"] == "bid" else asks).append(e["price"])
        med_bid = self._median_int(bids) if bids else None
        med_ask = self._median_int(asks) if asks else None
        return med_bid, med_ask

    @staticmethod
    def _median_int(lst: List[int]) -> int:
        s = sorted(lst)
        n = len(s)
        if n % 2 == 1:
            return s[n // 2]
    ...
    def _shadow_quotes(self, product: str, od: OrderDepth, pos: int,
        ...
        """Quote 1 tick INSIDE Mark 14's inferred bid/ask. Returns [] if Mark
        14 has no recent trades on either side (caller will fall back to OBI MM)."""
        ...
        if m14_bid is None and m14_ask is None:
            return []   # no fresh Mark 14 data — caller falls back

        limit = LIMITS[product]
        soft = int(SHADOW_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos

        # Bid: 1 tick better than Mark 14's bid, but never cross the book.
        ...
        if m14_bid is not None:
            ...
            if cand >= best_ask:
                cand = best_ask - 1
            if cand < best_bid:
                cand = best_bid + 1 if (best_bid + 1) < best_ask else best_bid
            our_bid = cand
        # Ask: 1 tick better (lower) than Mark 14's ask, never cross the book.
        ...
        if m14_ask is not None:
            ...
            if cand <= best_bid:
                cand = best_bid + 1
            if cand > best_ask:
                cand = best_ask - 1 if (best_ask - 1) > best_bid else best_ask
            our_ask = cand
