"""marco_rubio_v1 - hegseth velvetfruit/voucher + Hagrid HYDROGEL."""

# Asset constants must appear within the first 40 lines for the Rust MC
# simulator's symbol detector. Keep them at the top.
...
# Lineage notes (kept out of the module docstring so the Rust sim's first-40-
# line asset detector sees the constants above):
#   R4 backtest snapshot (2026-04-27):
#     - HYDROGEL: hegseth_v3 and Hagrid_v{1..5} produce identical R4 replay
#       PnL (3092 across 3 days) — same OBI-MM body, same parameters.
#     - VELVETFRUIT (R4 replay): hegseth_v3 9506 vs Hagrid_v1/v2 7112,
#       Hagrid_v3 5934. hegseth_v3 wins clearly.
#     - VEV vouchers (smile range): hegseth_v3's BS-smile composite take is
#       a structurally different signal (deviation-from-fair) — adopt it.
#   This file = hegseth_v3 with HYDROGEL routed through a Hagrid-style
#   _trade_hydrogel_obi_mm wrapper. Functionally identical to hegseth_v3 today
#   because the two OBI-MM bodies match, but split into named methods so
#   HYDROGEL and the basic vouchers (VEV_4000/4500) can diverge in future
#   tuning.
...
# Hagrid's per-voucher BS deltas (kept for sizing references; v1's voucher
# logic is hegseth-style smile, not velvet-z stack).
...
# Tried VEV_4500 in SMILE_VOUCHERS (2026-04-28): R4 replay barely moved
# (+297 to 28,383, V4500 itself stayed 0 — gates blocked) but MC tanked
# V4500 from +881 mean to -45,823 mean. Deep-ITM voucher prices near
# intrinsic don't fit the smooth BS smile, so the dev signal misfires in
# the simulator. Reverted.
SMILE_VOUCHERS = (VEV_5000, VEV_5100, VEV_5200, VEV_5300, VEV_5400, VEV_5500)
HYDROGEL_OBI_MM_ASSETS = (HYDROGEL,)
VOUCHER_OBI_MM_ASSETS = (VEV_4000, VEV_4500)
DEAD_ASSETS = (VEV_6000, VEV_6500)


class Trader:
    # ===== VELVETFRUIT MR (hegseth_v3 / v8 verbatim) =====
    EMA_ALPHA = 0.0005
    VAR_ALPHA = 0.01
    MR_K = 0.55
    MR_MAX_FRAC = 0.85
    ...
    # Velvet stop-loss (hegseth v8).
    STOP_LOSS_THRESHOLD = -2500
    STOP_DURATION_TICKS = 300
    STOP_POS_GATE = 0.80
    STOP_PAUSE_TICKS = 500

    # OBI tilt on velvet's MR passive layers.
    MR_OBI_FACTOR = 1.0
    MR_OBI_MULT_MIN = 0.3
    MR_OBI_MULT_MAX = 1.8

    # ===== OBI MM params (shared: HYDROGEL + VEV_4000 + VEV_4500) =====
    # Hagrid_v5 and hegseth_v3 use identical parameters here, so we keep one
    # set. If they ever need to diverge, split into HYDROGEL_* and VOUCHER_*.
    VEV_BASE_SIZE = 15
    ...
    # ===== Black-Scholes smile (hegseth v2 verbatim) =====
    ...
    # Voucher signal logic (hegseth_v3).
    ...
    # Voucher loose stop-loss (Hagrid_v5 — kept on the smile path too).
    ...
    def run(self, state: TradingState
        ...
        for product in state.order_depths:
            ...
            elif product in HYDROGEL_OBI_MM_ASSETS:
                # Hagrid HYDROGEL OBI MM.
                result[product] = self._trade_hydrogel_obi_mm(product, od, pos)
            elif product in VOUCHER_OBI_MM_ASSETS:
                # hegseth basic-voucher OBI MM (functionally identical to the
                # HYDROGEL path; kept distinct for future tuning).
                result[product] = self._trade_voucher_obi_mm(product, od, pos)
            else:
                result[product] = []  # VEV_6000 / VEV_6500 dead
        return result, 0, json.dumps(td)

    # ===== VELVET signal (hegseth) =========================================
    ...
    # ===== Smile machinery (hegseth v2 verbatim) ==========================
    ...
    # ===== Voucher composite signal (hegseth_v3) ==========================
    ...
    # ===== Hagrid HYDROGEL OBI MM =========================================
    def _trade_hydrogel_obi_mm(self, product: str, od: OrderDepth,
                                  pos: int) -> List[Order]:
        return self._obi_mm(product, od, pos)

    # ===== hegseth basic-voucher OBI MM (VEV_4000 / VEV_4500) =============
    def _trade_voucher_obi_mm(self, product: str, od: OrderDepth,
                                 pos: int) -> List[Order]:
        return self._obi_mm(product, od, pos)

    def _obi_mm(self, product: str, od: OrderDepth, pos: int) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
    ...
    # ===== Stop-loss helper (parameterised) ===============================
