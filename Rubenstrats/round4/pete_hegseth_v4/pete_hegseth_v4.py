"""pete_hegseth_v4 - v3 + Mark-gated takes on HYDROGEL / VEV_4000."""
...
# === Mark counterparty signals =========================================
# Filtered to confidence == "high" AND |drift_H200_mean| >= 5.0 (8 entries).
# Source: calibration/marks/signals.json. Drift is the average mid move
# in ticks over the 200 ticks following a Mark trade.
#   - Mark 14 (agg_pct=0): informed PASSIVE MM. Follow them.
#   - Mark 38 (agg_pct=100): informed AGGRESSOR. Fade them.
...
MARK_TAKE_SIZE = 5            # lots per take (best of 5/10/20 sweep in marks_b)
...
class Trader:
    ...
    def run(self, state: TradingState
        ...
        # Maintain rolling Mark trade log for HYDROGEL / VEV_4000.
        ...
        for product in state.order_depths:
            ...
            # ===== Priority 0: Mark-gated take layer (HYDROGEL, VEV_4000) =====
            ...
            # quote sizes against the worst-case post-fill (avoids the
            # position-limit cancellation rule).
    ...
    def _update_mark_trade_log(self, state: TradingState, td: dict) -> None:
        """Append fresh Mark-tagged market_trades (HYDROGEL / VEV_4000) to
        td['mark_trades'], dropping entries older than MARK_LOOKBACK_TICKS."""
        ...
        for sym, trades in (state.market_trades or {}).items():
            ...
            for t in trades:
                ...
                if not buyer and not seller:
                    continue
                # Only Marks we have signals for
    ...
    def _mark_take_for(self, product: str, td: dict, cur_ts: int
        ...
        """Return 'BUY' / 'SELL' if a Mark signal fires for `product`, else None.
    ...
    def _emit_mark_take(self, product: str, action: str, od: OrderDepth,
        ...
        """Emit a single cross-spread take of MARK_TAKE_SIZE lots in the
        signal direction. Returns (orders, signed_qty)."""
