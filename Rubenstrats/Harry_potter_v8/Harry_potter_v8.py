"""Harry_potter_v8 — v7 + HYDROGEL on multi-level MR.

v7 portal: +17,404 (v4 + OBI tilt — flat on portal, small CSV lift).

The critical finding from v7's portal log: fills are capped by bot/
taker volume at each price level, not by our posted size. v4/v6/v7
had PnL identical to the penny on 9/10 assets across submissions.
Scaling size at the same price doesn't help.

v8 adds price-level diversity: HYDROGEL (15-tick bot spread) gets 4
passive levels across its spread (bid+1, +3, +5, +7) with decreasing
sizes. Tight-spread assets stay at 2 levels inside. Also switches
HYDROGEL's strategy from OBI MM to full MR (target position from
z-score) — the same logic that powered VELVETFRUIT + VEVs to 17k.

Earlier MR-on-HYDROGEL tests used VELVETFRUIT's tight-spread 2-level
execution and gave only 615 CSV (vs OBI MM's 6,484). The multi-level
profile is HYDROGEL-specific and should let passive orders populate
the bot's entire spread.

v7's OBI tilt on the big side is preserved — same logic on every MR
asset, per-asset layer configs just change WHERE and HOW BIG the
layers are.

Everything else unchanged: slow EMA (alpha=0.0005), K=0.55,
MAX_FRAC=0.85, position-gated SL.

--- v7 doc preserved below ---
...
# Per-asset passive-layer offsets (ticks inside spread) + big-side sizes.
# Tight-spread assets cap at 2 inside-spread levels; HYDROGEL's 15-tick
# bot spread makes room for 4 levels at bid+1/+3/+5/+7.
# CSV-tuned per-asset passive-layer config. Each (offset, big_size) tuple
# defines one layer inside the spread. HYDROGEL's taker-flow capacity caps
# at ~12-15 per tick at the penny-jump price, so a single level at bid+1
# with size 15 extracts more than either multi-level (wasted orders at
# prices with no taker flow) or bigger sizes (over-saturates the level).
# Tight-spread assets (VELVETFRUIT, VEVs) have more reliable deeper fills
# so 2 levels at size 30 each works well.
...
MR_ASSETS = (HYDROGEL, VELVETFRUIT, VEV_5000, VEV_5100, VEV_5200, VEV_5300, VEV_5400, VEV_5500)
VEV_MM_ASSETS = (VEV_4000, VEV_4500)
OBI_MM_ASSETS = ()  # HYDROGEL moved to MR in v8
...
class Trader:
    ...
    def _trade_mr(self, product: str, od: OrderDepth, pos: int, td: dict,
        ...
        # Per-asset passive-layer config. Each (offset, big_size) pair defines
        # one level inside the spread. The small side gets a smaller standard
        # size (max(5, big//3)) — opportunistic liquidity, not a signal bet.
        layers = LAYER_CONFIG.get(product, [(1, self.MR_MM_LEVEL_SIZE),
                                            (2, self.MR_MM_LEVEL_SIZE)])

        # OBI tilt on the MR-favored side (v7 logic). Confluence amplifies,
        # conflict shrinks. When there's no strong MR target, apply OBI bias
        # to both small sides as pure OBI MM.
        ...
        for offset, big_sz in layers:
            ...
            if diff_after > 0:
                ...
                b_each = int(big_sz * mult)
                a_each = small_sz
            elif diff_after < 0:
                mult = max(self.MR_OBI_MULT_MIN,
                           min(self.MR_OBI_MULT_MAX, 1.0 + self.MR_OBI_FACTOR * (-obi)))
                a_each = int(big_sz * mult)
                b_each = small_sz
            else:
