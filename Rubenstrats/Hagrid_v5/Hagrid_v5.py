...
MC backtest revealed: Hagrid_v3 portal SR = +1.15 (one realized path),
MC across-session SR = −1.01 (mean −39k, 5%-95% range [−106k, +18k]).
The portal session is in the favorable ~10% of the FV-path
distribution; the typical session loses heavily because vouchers stack
into runaway-drift sessions with no cap. Same failure mode as v8's
HYDROGEL collapse (−6,725) but applied to all 6 stacked vouchers
simultaneously.

v5 keeps the v3 architecture (proven +25,801 on portal) and adds a
LOOSE voucher SL designed to:
  - NEVER fire on the v3 portal session (which converges within ~−2k
    MTM during build-up before reverting)
  - DOES fire on adverse-drift sessions where MR loads up and the
    underlying continues drifting

Settings:
  - VOUCHER_STOP_POS_GATE = 0.80 → gate at 240 contracts
    (matches velvet's 0.80; arms once stack load is reached)
  - VOUCHER_STOP_THRESHOLD = −5,000 (vs velvet's −2,500)
    (2× deeper — won't fire on normal MR build-up dips that hit ~−2k,
    but will fire on sustained adverse drift)
  - VOUCHER_STOP_DURATION = 500 ticks (vs velvet's 300)
    (1.67× longer — filters v1-style transient drawdowns)
  - VOUCHER_STOP_PAUSE = 500 ticks

Expected outcome:
  - Same portal +25,801 (gate doesn't arm, SL doesn't fire)
  - On adverse-drift session: caps the per-voucher loss at roughly
    −5k instead of allowing it to bleed to −15k+
  - MC mean should improve from −39k toward 0 or positive

This is insurance — pays nothing on the proven good path, protects
against the unseen final-round eval session that may differ.
...
class Trader:
    ...
    STACK_TAKE_MAX = 80         # max contracts taken aggressively per tick (vouchers)
    ...
    # -------- Position-gated stop-loss (VELVETFRUIT) --------
    STOP_LOSS_THRESHOLD = -2500
    STOP_DURATION_TICKS = 300
    STOP_POS_GATE = 0.80
    STOP_PAUSE_TICKS = 500

    # -------- Loose voucher SL (insurance against runaway drift) --------
    # Designed to:
    #   - NOT fire on v3 portal session (per-voucher MTM peak drawdown
    #     stays around −2k during normal MR build-up; −5k threshold has
    #     2× safety margin)
    #   - FIRE on adverse-drift sessions where MTM stays below −5k for
    #     500+ consecutive ticks (sustained drift, not noise)
    # Gate matches velvet's 0.80 — arms when stacked load is reached.
    # Threshold is 2× deeper than velvet's −2,500.
    # Duration is 1.67× longer than velvet's 300 to filter v1-style
    # transient drawdowns during normal MR build-up.
    ...
    def _trade_voucher(self, sym: str, od: OrderDepth, pos: int,
        ...
        # Loose SL — gate=0.95×300=285 sits above the max stack target
        # (271 at 0.90 frac), so this never arms during normal stacking.
        # Threshold −5,000 / duration 500 ensure it only triggers on
        # genuine runaway-drift sessions, not normal MR build-up dips.
        if self._sl_check(td, sym, pos, mid, limit, timestamp,
                          threshold=self.VOUCHER_STOP_THRESHOLD,
                          duration=self.VOUCHER_STOP_DURATION,
                          gate=self.VOUCHER_STOP_POS_GATE,
                          pause=self.VOUCHER_STOP_PAUSE):
    ...
    def _sl_check(self, td: dict, product: str, pos: int, mid: float,
                  limit: int, timestamp: int,
                  threshold: float = None, duration: int = None,
                  gate: float = None, pause: int = None) -> bool:
        """Returns True if SL pause is currently active for this product.

        Optional params override defaults — used for the looser voucher SL.
        """
        thr = threshold if threshold is not None else self.STOP_LOSS_THRESHOLD
        dur = duration if duration is not None else self.STOP_DURATION_TICKS
        gt = gate if gate is not None else self.STOP_POS_GATE
        pa = pause if pause is not None else self.STOP_PAUSE_TICKS
        ...
        pos_gate_met = abs(pos) >= int(gt * limit)
        if (below >= dur and pos_gate_met and pause_until < timestamp):
            td[f"pause_until_{product}"] = timestamp + pa
            return True
