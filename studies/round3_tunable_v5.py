"""Tunable v5 — porush + VELVET OBI + deep-ITM OBI + MAF. See final_audit.md."""
...
# v5: VELVETFRUIT extracted from MR_ASSETS (uses dedicated VEL_OBI handler).
#     VEV_4000/4500 also handled by deep-ITM OBI (replaces _trade_vev_mm).
#     VEV_5500 still goes through _trade_vev_mm (stratton-style).
MR_ASSETS = (VEV_5000, VEV_5100, VEV_5200, VEV_5300, VEV_5400)
DEEP_ITM_ASSETS = (VEV_4000, VEV_4500)
VEV_MM_ASSETS = (VEV_5500,)


class Trader:
    # === Locked from search 4 OOS winner (porush / trial #119) ============
    # MR core (jordan/search-3)
    ...
    # Stratton OBI MM tier sizes (for VEV_5500 fallback)
    VEV_BASE_SIZE = 3
    ...
    # === NEW v5 — VELVETFRUIT confidence-sized OBI handler ================
    # Defaults from final_audit.md Sec 1 recommendations.
    VEL_OBI_THRESH = 0.30
    VEL_OBI_SIZE_MAX = 60
    VEL_OBI_SIZE_K_CONF = 0.7
    VEL_OBI_SIZE_MIN = 8
    VEL_MM_BASE_SIZE = 18
    VEL_OBI_SKEW_TICKS = 0   # spread is ~5 — skew via size only, not quote level
    VEL_SOFT_POS_FRAC = 0.6
    VEL_TIGHT_SPREAD_MIN = 2

    # === NEW v5 — VEV_4000 / VEV_4500 deep-ITM OBI handler =================
    # Per-asset SIZE_MAX since 4000 has wider spread / bigger drift than 4500.
    V4K_OBI_THRESH = 0.30
    V4K_OBI_SIZE_MAX = 120
    V4K_OBI_SIZE_K_CONF = 0.8
    V4K_OBI_SKEW_TICKS = 1
    V4K_MM_BASE_SIZE = 25
    V45_OBI_SIZE_MAX = 90       # VEV_4500-only override
    V4_OBI_SIZE_MIN = 8
    V4_SOFT_POS_FRAC = 0.6
    V4_TIGHT_SPREAD_MIN = 2

    # MAF bid (R3) — final_audit.md recommends 500
    MAF_BID = 500

    def __init__(self):
        overrides = _load_param_overrides()
        for k, v in overrides.items():
            if k in ("OBI_CONFIRM_TAKE", "DISABLE_TAKES"):
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == HYDROGEL:
                ...
                result[product] = self._trade_velvet_obi(od, pos, td)
            elif product in DEEP_ITM_ASSETS:
    ...
    # === NEW v5: VELVETFRUIT confidence-sized OBI ========================
    def _trade_velvet_obi(self, od: OrderDepth, pos: int, td: dict) -> List[Order]:
        ...
        # Inventory-bleed via small MR (same EMA as previous, but no position
        # target — just MM with mild OBI confidence sizing). Keep ema state
        # warm so we have FV-tracking context.
        mid = (best_bid + best_ask) / 2.0
        self._update_ema(td, VELVETFRUIT, mid)
        ...
        if abs_obi >= self.VEL_OBI_THRESH:
            conf_size = max(self.VEL_OBI_SIZE_MIN,
                             min(self.VEL_OBI_SIZE_MAX,
                                 int(math.ceil(limit * abs_obi * self.VEL_OBI_SIZE_K_CONF))))
            if obi > 0:
                buy_size = conf_size
                sell_size = self.VEL_MM_BASE_SIZE
                if self.VEL_OBI_SKEW_TICKS > 0:
                    our_ask = best_ask + self.VEL_OBI_SKEW_TICKS - 1
            else:
                buy_size = self.VEL_MM_BASE_SIZE
                sell_size = conf_size
                if self.VEL_OBI_SKEW_TICKS > 0:
                    our_bid = best_bid - self.VEL_OBI_SKEW_TICKS + 1
        else:
            buy_size = self.VEL_MM_BASE_SIZE
            sell_size = self.VEL_MM_BASE_SIZE
    ...
    # === NEW v5: VEV_4000/4500 deep-ITM confidence-sized OBI =============
    def _trade_deep_itm_obi(self, product: str, od: OrderDepth, pos: int) -> List[Order]:
        bids = sorted(od.buy_orders.keys(), reverse=True) if od.buy_orders else []
        asks = sorted(od.sell_orders.keys()) if od.sell_orders else []
        if not bids or not asks:
            return []
        best_bid = bids[0]
        best_ask = asks[0]
        if best_ask - best_bid < self.V4_TIGHT_SPREAD_MIN:
            return []
        b1_vol = od.buy_orders[best_bid]
        a1_vol = -od.sell_orders[best_ask]
        total = b1_vol + a1_vol
        ...
        soft_thresh = int(self.V4_SOFT_POS_FRAC * limit)
        buy_room = limit - pos
        sell_room = limit + pos
        size_max = self.V45_OBI_SIZE_MAX if product == VEV_4500 else self.V4K_OBI_SIZE_MAX

        our_bid = best_bid + 1
        our_ask = best_ask - 1
        if abs_obi >= self.V4K_OBI_THRESH:
            conf_size = max(self.V4_OBI_SIZE_MIN,
                             min(size_max,
                                 int(math.ceil(limit * abs_obi * self.V4K_OBI_SIZE_K_CONF))))
            if obi > 0:
                buy_size = conf_size
                sell_size = self.V4K_MM_BASE_SIZE
                if self.V4K_OBI_SKEW_TICKS > 0:
                    our_ask = best_ask + self.V4K_OBI_SKEW_TICKS - 1
            else:
                buy_size = self.V4K_MM_BASE_SIZE
                sell_size = conf_size
                if self.V4K_OBI_SKEW_TICKS > 0:
                    our_bid = best_bid - self.V4K_OBI_SKEW_TICKS + 1
        else:
            # Fall through to stratton tier sizing for low-OBI ticks
            buy_size, sell_size = self._vev_obi_sizes(obi)
            if self.V4K_MM_BASE_SIZE > self.VEV_BASE_SIZE:
                if obi > 0:
                    buy_size = max(buy_size, self.V4K_MM_BASE_SIZE)
                else:
                    sell_size = max(sell_size, self.V4K_MM_BASE_SIZE)
    ...
    # === Stratton MR (for VEV_5000-5400, VELVETFRUIT moved out) ==========
