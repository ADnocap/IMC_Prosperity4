"""Tunable v4 — HYDROGEL-focused. Locks VELVET/voucher at v3 trial #300.
See analysis/round3/hydrogel_deep.md for the new HYDROGEL handler design."""
...
class Trader:
    # === Locked from v3 search OOS winner (trial #300) ====================
    EMA_ALPHA = 4.308428675778431e-05
    VAR_ALPHA = 0.03588256506509171
    MR_K = 0.04481152690538941             # v3 trial 300 (OOS winner)
    ...
    OBI_SKEW_T1_THRESH = 0.08480914085182328   # v3 trial 300
    OBI_SKEW_T2_THRESH = 0.7984188975781722
    OBI_SKEW_T1_TICKS = 0
    OBI_SKEW_T2_TICKS = 2
    BASE_MM_SIZE = 37                          # v3 trial 300

    # === HYDROGEL-only NEW (per hydrogel_deep.md Sec 7) ===================
    # Layer A: OBI passive skew (workhorse) — confidence-sized
    HY_OBI_THRESH = 0.10                # |L1-OBI| trigger to skew
    HY_OBI_SKEW_TICKS = 2               # quote pull-away on unfavoured side
    HY_OBI_SIZE_K_CONF = 1.0            # size = ceil(LIMIT * |OBI| * k)
    HY_OBI_SIZE_MIN = 5                 # floor on confidence-sized quote
    HY_OBI_SIZE_MAX = 100               # cap
    HY_MM_BASE_SIZE = 30                # passive size when |OBI| < HY_OBI_THRESH

    # Layer B: rev_z + OBI agreement-gate take (NOT take-all-spread)
    # Defaults DISABLED via threshold=99 — search turns it on.
    HY_REVZ_WINDOW = 500                # Sec 1: 500-tick window dominates 50
    HY_REVZ_THRESHOLD = 99.0            # |z| trigger (search will lower)
    HY_REVZ_OBI_GATE = 0.05             # |OBI| min for gate
    HY_REVZ_TAKE_SIZE = 0               # default 0 = disabled (search opens)
    HY_REVZ_HOLD = 300                  # tick to natural-bleed

    # Soft position bleed for HYDROGEL specifically
    HY_SOFT_POS_FRAC = 0.6
    ...
    def run(self, state: TradingState) -> Tuple[Dict[str, List[Order]], int, str]:
        ...
        for product in state.order_depths:
            ...
            if product == HYDROGEL:
                result[product] = self._trade_hydrogel_v4(od, pos, td)
    ...
    # =====================================================================
    # NEW HYDROGEL handler per hydrogel_deep.md Sec 7
    # =====================================================================
    def _trade_hydrogel_v4(self, od: OrderDepth, pos: int, td: dict) -> List[Order]:
        ...
        # --- Maintain rev_z buffer (window = HY_REVZ_WINDOW) ----------
        buf_key = "hy_mids"
        age_key = "hy_revz_age"
        ...
        take_qty = 0  # +ve = buy take, -ve = sell take
        if (len(buf) >= win
                and self.HY_REVZ_TAKE_SIZE > 0
                and self.HY_REVZ_THRESHOLD < 50.0):
            ...
            if std > 0:
                z = (mid - mean) / std
                # Trigger conditions:
                # 1) |z| > HY_REVZ_THRESHOLD
                # 2) |OBI| > HY_REVZ_OBI_GATE
                # 3) sign(-z) == sign(OBI)  -- OBI confirms reversion direction
                if abs(z) > self.HY_REVZ_THRESHOLD and abs_obi > self.HY_REVZ_OBI_GATE:
                    desired_dir = -1 if z > 0 else 1   # mid high -> sell -> -1
        ...
        # Bleed natural via passive MM after HOLD ticks (no aggressive unwind)
        ...
        # Quotes at penny-jump by default; pull unfavoured side away by
        # HY_OBI_SKEW_TICKS when |OBI| > HY_OBI_THRESH.
        our_bid = best_bid + 1
        our_ask = best_ask - 1
        if abs_obi >= self.HY_OBI_THRESH:
            # Confidence sizing for the favoured side
            conf_size = max(self.HY_OBI_SIZE_MIN,
                             min(self.HY_OBI_SIZE_MAX,
                                 int(math.ceil(limit * abs_obi * self.HY_OBI_SIZE_K_CONF))))
            if obi > 0:
                # Mid expected up; favour the BID (buying)
                buy_size = conf_size
        ...
        # Account for take_qty already used
    ...
    # =====================================================================
    # Stratton MR + OBI MM (UNCHANGED from v3 trial 301)
    # =====================================================================
