...
pub fn flag_specs() -> Vec<FlagSpec> {
    vec![FlagSpec {
        ...
        kind: FlagKind::Path,
        default: None,
        help: "Replay observed FV (JSON: flat [f64] or object with 'hydrogel_pack' key)",
    }]
}
...
pub fn build(flags: &HashMap<String, String>) -> Result<Box<dyn AssetSim>> {
    let replay_fv = match flags.get("replay-fv") {
        Some(raw) => {
            let path = parse_path("hydrogel-pack-replay-fv", raw)?;
            Some(load_replay_fv_path(&path, "hydrogel_pack")?)
        }
        None => None,
    };
    Ok(Box::new(HydrogelPack { replay_fv }))
}
...
impl AssetSim for HydrogelPack {
    fn symbol(&self) -> &str {
        SYMBOL
    }
    ...
    fn simulate_fv(
        ...
        let _ = day_index;
        let sigma: f64 = 1.9159433588411283;
        let drift: f64 = 0.0;
        let mean: f64 = 9979.317159933371;
    ...
    fn make_book(&self, fair: f64, rng: &mut ChaCha8Rng) -> Book {
        ...
        let v1 = rng.gen_range(20..=30);
        if rng.gen_bool(0.999) { bids.push(((fair * (1.0 - 0.0010000958106489567)).floor() as i32, v1)); }
        if rng.gen_bool(0.999) { asks.push(((fair * (1.0 + 0.0010000958106489567)).ceil() as i32, v1)); }
        // Bot 2: bid_rate=0.975 ask_rate=0.980  sides_tied=True
        let v2 = rng.gen_range(10..=15);
        if rng.gen_bool(0.975) { bids.push(((fair - 0.5).floor() as i32 - 7, v2)); }
        if rng.gen_bool(0.98) { asks.push((banker_round(fair) as i32 + 8, v2)); }
    ...
    fn sample_trade_qty(
        ...
        let _ = market_buy;
        let values: &[i32] = &[2, 3, 4, 5, 6];
        let weights: &[u32] = &[193, 198, 202, 212, 205];
