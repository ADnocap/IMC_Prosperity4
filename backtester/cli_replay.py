...
from backtester.file_reader import FileReader, FileSystemReader
...
def resolve_algorithm_path(algorithm: Path) -> Path:
    """If the algorithm file doesn't exist, try traders/round{4,3,2,1}/, traders/."""
    if algorithm.exists():
        return algorithm
...
def parse_algorithm(algorithm: Path) -> Any:
    ...
    for p in [str(algorithm.parent), str(algorithm.parent.parent)]:
...
def default_data_root() -> Path:
    """Default data directory: data/prosperity4 relative to project root."""
    return Path(__file__).resolve().parents[1] / "data" / "prosperity4"


def parse_data(data_root: Optional[Path]) -> FileReader:
    if data_root is not None:
        return FileSystemReader(data_root)
    return FileSystemReader(default_data_root())
