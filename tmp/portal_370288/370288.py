"""Harry_potter_v4 — mean-reversion on HYDROGEL + VELVETFRUIT, OBI-tilted MM on VEVs.

Pivot from pure MM: HYDROGEL (AR(1) phi=0.998, half-life 300 ticks) and
VELVETFRUIT (same) are slightly mean-reverting random walks. Use the full
position limit (200 each) to take directional bets on reversion.

Strategy per delta-1 asset (HYDROGEL, VELVETFRUIT):
  1. Maintain EMA fair value + rolling std via traderData.
  2. Compute z = (mid - FV) / sigma.
  3. Target position = -K * z * limit (clip to +/- 0.85 * limit).
  4. Execute passively: post MM-style quotes but SIZED toward target.
     - If target > pos (want to buy more): post big bid, small ask.
     - If target < pos (want to sell more): post big ask, small bid.
  5. If |z| > MR_TAKE_Z (strong signal), cross-spread up to TAKE_MAX to
     lock in position quickly. Spread-crossing cost is the premium we pay
     for immediate size.

Strategy on VEVs: keep v3's OBI-tilted penny-jump MM.
Dead VEV_6000/6500: skip.

State persists in traderData:
  ema_<asset>  : EMA of mid, alpha = EMA_ALPHA
  var_<asset>  : EMA of (mid - prev_ema)^2 for variance estimate
  n_<asset>    : tick counter for warmup gating
"""
