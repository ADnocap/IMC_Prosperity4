...
class Trader:
    ...
    def _trade_ash(self, od: OrderDepth, position: int, td: dict) -> List[Order]:
        ...
        # Phase 1: Conservative taking (Timo-style)
        # Take asks below FV (1+ tick edge). At exactly FV, only take to reduce inventory.
        for ask_price in asks_sorted:
            ...
            if ask_price == fv_r and position >= 0:
                # At FV: only take if short (reduces inventory)
                continue
        ...
        for bid_price in bids_sorted:
            ...
            if bid_price == fv_r and position <= 0:
                # At FV: only take if long (reduces inventory)
                continue
            orders.append(Order("ASH_COATED_OSMIUM", bid_price, -can))
            sell_ordered += can

        # Phase 2: Penny-jump with Bot3 detection — no inventory skew
