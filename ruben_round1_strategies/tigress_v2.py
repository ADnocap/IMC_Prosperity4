"""tigress: Step-reversal edge-1 taking on OSMIUM.

Portal finding: Bot1 asymmetry is noise (50.8%), step reversal is 84.5%.
Only change from alex: take at edge >= 1 on the reversal side.
All clearing and passive quoting identical to alex (zero structural risk).

After FV steps DOWN: expect UP   -> buy at edge >= 1
After FV steps UP:   expect DOWN -> sell at edge >= 1
Otherwise: standard edge >= 2 (alex behavior)"""
...
class Trader:
    ...
    # ------------------------------------------------------------------
    # OSMIUM: alex base + step-reversal edge-1 taking
    # ------------------------------------------------------------------
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Reversal: last step up -> expect down (bearish), last step down -> expect up (bullish)
        bullish = signal < 0
        bearish = signal > 0
        ...
        # === TAKE: edge >= 1 on reversal side, >= 2 otherwise ===
        ...
        # === CLEAR: identical to alex (clear ALL at FV) ===
