...
class Trader:
    ...
    Pure v2 with one change: ACO passive quote steps 2 ticks inside the
    wall instead of 1 (our_bid = ref_bid + 2 -> fv-6 instead of fv-7;
    mirror on ask). Gives up 1 XIREC edge per fill to cross into more
    counterparty orders.

    Hypothesis: portal fill rate is low because our quote sits 1 tick
    inside the Bot2 wall at fv+/-8, so fv+/-7 is still further from the
    midpoint than where Bot3 noise orders live (fv+/-1..+/-3). Tightening
    to fv+/-6 catches more Bot3 trades and any passing flow.

    Tradeoff: edge-per-fill drops 7 -> 6, so fill rate needs to rise by
    ~17% to break even. Sim-measurable.

    ACO: Alex Take/Clear/Make with Bot1-anchored FV + V4/V5 L2 wall
         detection for penny-jump.
    IPR: TaiLUNG v1 IPR (accumulate + passive bid + FV-aware passive ask).
    """
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        our_bid = min(ref_bid + 2, fv_r - 1)
        our_ask = max(ref_ask - 2, fv_r + 1)
