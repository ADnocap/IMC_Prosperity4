"""po_v3: Exact alex base + Bot1 edge-1 taking.

Identical to alex's zero-inventory MM except:
  When Bot1 asymmetry signal is detected (92-95% accurate),
  take at edge >= 1 instead of >= 2 on the confirmed side.
Everything else (clearing, passive caps, penny-jump) is untouched."""
...
class Trader:
    ...
    # ------------------------------------------------------------------
    # Bot1 asymmetry detection
    # ------------------------------------------------------------------
    def _detect_bot1(self, od, bids, asks, fv):
        """Returns +1 (bullish), -1 (bearish), 0 (no signal)."""
        bid_off = ask_off = None
        for p in bids:
    ...
    # ------------------------------------------------------------------
    # FV estimation (identical to alex)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # OSMIUM trading (alex base + Bot1 edge-1 taking)
    # ------------------------------------------------------------------
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Detect Bot1 signal
        bot1 = self._detect_bot1(od, bids, asks, fv_r)

        buy_ordered = 0
        sell_ordered = 0

        # TAKE: edge >= 2 normally.
        # With Bot1 confirmation: edge >= 1 on the confirmed side.
        buy_edge = 1 if bot1 > 0 else 2    # Bot1 bullish → buy at FV-1
        sell_edge = 1 if bot1 < 0 else 2    # Bot1 bearish → sell at FV+1
        ...
        # AGGRESSIVE CLEAR: flatten ALL inventory at FV or better (identical to alex)
        ...
        # MAKE: penny-jump with capped size (identical to alex)
        buy_room = LIMIT - starting_pos - buy_ordered
        sell_room = LIMIT + starting_pos - sell_ordered

        max_passive = 40
        buy_room = min(buy_room, max_passive)
        sell_room = min(sell_room, max_passive)
