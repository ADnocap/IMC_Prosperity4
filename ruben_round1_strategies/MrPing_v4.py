...
# ACO micro skew controls (stricter gating).
...
class Trader:
    """
    Round 1 — TaiLUNG v2 + strictly gated OBI skew.
    ...
    def _obi_skew(self, od: OrderDepth, bids: List[int], asks: List[int], pos_after: int) -> int:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        center = fv_r + self._obi_skew(od, bids, asks, pos_after)
