...
class Trader:
    ...
    Pure v2 with one change: ACO MAKE cap 40 -> 50.

    Portal log (sub 236340) showed max ASH position +55, never touched
    the |80| hard limit; the 40 cap was leaving volume on the table.
    Signal-skew experiments (v5/v6) regressed portal, so v7c isolates
    a single structural lever.

    ACO: Alex Take/Clear/Make with Bot1-anchored FV + V4/V5 L2 wall
         detection for penny-jump (skips Bot3 noise at L1).
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Phase 3: MAKE - V4/V5 L2 wall detection + penny-jump, capped at 50.
