"""tigress: Partial-clear OSMIUM MM.

Key insight: 62% of alex's MTM PnL comes from large-position ticks (>20 units).
Passive fills at FV-7 are naturally contrarian (buy dips, sell rallies).
Instead of clearing ALL inventory at FV, keep some natural exposure.

Only change from alex: clear to KEEP_POS instead of 0.
No signal dependency. Works in MC (passive fills are contrarian by nature)."""
...
KEEP_POS = 10  # Keep up to this many units instead of clearing to 0
...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # === CLEAR: partial — keep up to KEEP_POS in either direction ===
        pos_after = starting_pos + buy_ordered - sell_ordered

        if pos_after > KEEP_POS:
            # Clear excess above KEEP_POS
            to_clear = pos_after - KEEP_POS
            for bp in bids:
                if bp < fv_r:
                    break
                vol = od.buy_orders[bp]
                c = min(vol, to_clear, LIMIT + starting_pos - sell_ordered)
                if c > 0:
        ...
        elif pos_after < -KEEP_POS:
            # Clear excess below -KEEP_POS
            to_clear = -pos_after - KEEP_POS
            for ap in asks:
                if ap > fv_r:
                    break
                vol = -od.sell_orders[ap]
                c = min(vol, to_clear, LIMIT - starting_pos - buy_ordered)
                if c > 0:
        ...
        # === PASSIVE: identical to alex (penny-jump, capped at 40) ===
