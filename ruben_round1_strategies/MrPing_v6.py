...
class Trader:
    """
    Round 1 — TaiLUNG v2 + strict OBI skew with late-session cutoff.
    Adds small OBI-gated edge-1 taking with hard caps.

    ACO: Alex Take/Clear/Make with Bot1-anchored FV + V4/V5 L2 wall
         detection for penny-jump (skips Bot3 noise at L1), plus a tiny
         1-tick OBI center shift only when imbalance is strong and inventory
         is light.
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict, timestamp: int):
        ...
        extra_buy_room = 0
        extra_sell_room = 0
        if bids and asks and abs(starting_pos) <= ACO_OBI_TAKE_INV_GATE:
            best_bid = bids[0]
            best_ask = asks[0]
            bid_vol = od.buy_orders.get(best_bid, 0)
            ask_vol = -od.sell_orders.get(best_ask, 0)
            ...
            if total > 0:
                obi = (bid_vol - ask_vol) / total
                if obi >= ACO_OBI_TAKE_TRIGGER:
                    buy_take_edge = 1
                    extra_buy_room = ACO_OBI_TAKE_MAX
                elif obi <= -ACO_OBI_TAKE_TRIGGER:
                    sell_take_edge = 1
                    extra_sell_room = ACO_OBI_TAKE_MAX
