...
class Trader:
    ...
    ACO_ANCHOR_MAX = 2
    ACO_STEP_SKEW = 1        # fade magnitude for last FV step (65% reversal per analysis §2)
    ACO_TOTAL_SKEW_MAX = 3   # cap on combined OBI + anchor + step skew
    ACO_RET_WINDOW = 16
    ACO_RECENT_RET = 4
    ACO_VOL_WIDEN = 1.35
    ...
    def _aco_make_signal(self, od: OrderDepth, bids: List[int], asks: List[int], fv: float, td: dict):
        ...
        # Additive composition: three independent signals summed and capped.
        combined = obi_skew + anchor_skew + step_skew
