"""Hybrid D: Alex base + V4 edge-0 inventory clearing takes.
Takes at FV to reduce inventory (buy at FV if short, sell at FV if long).
Alex's clearing still runs after. This adds extra clearing opportunities."""
...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # TAKE: edge >= 2 (alex) PLUS edge-0 for inventory reduction (V4)
        for ap in asks:
            if ap > fv_r: break
            vol = -od.sell_orders[ap]
            can = LIMIT - starting_pos - buy_ordered
            if can <= 0: break
            if ap == fv_r and position >= 0:
                # At FV: only buy to reduce short position
                continue
            if ap > fv_r - 2 and ap < fv_r:
                # Edge 1: skip (not profitable enough)
                continue
            qty = min(vol, can)
        ...
        for bp in bids:
            ...
            if can <= 0: break
            if bp == fv_r and position <= 0:
                # At FV: only sell to reduce long position
                continue
            if bp < fv_r + 2 and bp > fv_r:
                # Edge 1: skip
                continue
            qty = min(vol, can)
        ...
        # PASSIVE: alex penny-jump, capped at 40
