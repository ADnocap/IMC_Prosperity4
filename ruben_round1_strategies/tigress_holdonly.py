"""tigress_holdonly: Alex base + signal-aware clearing ONLY.
No edge-1 taking, no tight directional quotes.
Isolates the impact of holding positions with Bot1 signal."""
...
class Trader:
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # TAKE: edge >= 2 always (alex standard, no edge-1)
        for ap in asks:
            if ap > fv_r - 2:
                break
        ...
        # CLEAR: signal-aware (the only change from alex)
            ...
            if pos_after < 0:
                for ap in asks:
                    ...
                    if c > 0:
                        ...
                        buy_ordered += c
                        pos_after += c
            elif pos_after > MAX_HOLD:
                excess = pos_after - MAX_HOLD
                for bp in bids:
                    if bp < fv_r:
                        break
                    vol = od.buy_orders[bp]
