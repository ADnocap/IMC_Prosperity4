...
class Trader:
    ...
    v4 base plus mean-reversion-to-mu skew stacked on Bot1 asymmetry.

    ACO signal composition (all gated by inventory guards):
      * Bot1 asymmetry drives adaptive learner (+1/-1 after hysteresis)
      * Mean-reversion to mu = 10000 when |fv - mu| >= MR_DEADZONE
      * Combined skew in {-2,-1,0,+1,+2} adjusts qty caps; at |2|
        the favored-side price is shifted 1 tick inside the wall.
    """

    # Adaptive Bot1 signal controls (unchanged from v4).
    ...
    MR_MU = 10000
    MR_DEADZONE = 4          # |fv - mu| < this => no mean-rev skew
    MR_STRONG = 8            # |fv - mu| >= this strengthens nothing extra (hook)

    # ACO safety caps.
    ACO_SOFT_POS = 30
    ACO_HARD_POS = 55        # beyond this, drop all skew

    # IPR exits (unchanged from v4).
    IPR_ASK_OFFSET_1 = 8
    IPR_ASK_OFFSET_2 = 9
    IPR_ASK_THRESH_1 = 60
    IPR_ASK_THRESH_2 = 75
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Default symmetric caps.
        buy_cap = 40
        sell_cap = 40

        # Hard-disable skew at extreme inventory — let clear logic work.
        skew_allowed = abs(pos_after) < self.ACO_HARD_POS

        if skew_allowed and combined != 0:
            # Qty skew: 1 unit per signal level.
            buy_cap = 40 + combined
            sell_cap = 40 - combined

            # At full-strength combined signal, tighten favored price 1 tick.
            if combined >= 2:
                # Strong bullish: shift bid closer to FV, keep ask.
                our_bid = min(our_bid + 1, fv_r - 1)
            elif combined <= -2:
                # Strong bearish: shift ask closer to FV, keep bid.
                our_ask = max(our_ask - 1, fv_r + 1)

        # Soft inventory guard (v4 behaviour, now triggered by any non-zero combined).
    ...
    # ==================================================================
    # INTARIAN_PEPPER_ROOT (unchanged from v4)
    # ==================================================================
