...
class Trader:
    ...
    New:
      ...
      - Tiny one-sided price skew with strict safety caps.
    ...
    def _trade_osmium(self, od: OrderDepth, position: int, td: dict):
        ...
        # Phase 3: MAKE — baseline quotes + tiny one-sided price skew
        ...
        if signal > 0 and abs(pos_after) < 15:
            # Bullish: only tighten bid by 1 tick.
            our_bid = min(base_bid + 1, fv_r - 1)
        elif signal < 0 and abs(pos_after) < 15:
            # Bearish: only tighten ask by 1 tick.
            our_ask = max(base_ask - 1, fv_r + 1)
